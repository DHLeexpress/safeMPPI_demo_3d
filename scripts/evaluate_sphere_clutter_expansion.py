"""Evaluate randomized three-sphere expansion checkpoints.

Metrics use disjoint raw temperature-1 rollouts. A separately fixed randomized
three-sphere scene supports visual comparison across checkpoints, while the
...
def main():
    ...
    if not (
        ...
        parser.error(
            "three-sphere evaluation requires a visual lab pretrained policy"
        )
    ...
    if not task_config_path.is_file():
        raise FileNotFoundError(
            "three-sphere expansion requires task_config_resolved.json"
        )
    ...
    if minimum_count != 3 or int(scene_spec.max_count) != 3:
        parser.error(
            "Stage-2 evaluator requires exactly three randomized spheres"
        )
    evaluate_lab_clutter_expansion(
