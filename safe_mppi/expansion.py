...
class ExpansionConfig:
    ...
    def validate(self) -> None:
        ...
        if self.successful_trajectory_selector not in {
                "lowest_episode_id", "max_above_fraction", "max_mean_z"}:
            raise ValueError(
                "successful_trajectory_selector must be lowest_episode_id or "
                "max_above_fraction or max_mean_z"
            )
...
def _run_safe_expansion_impl(
    ...
    *,
    ...
    if config.first_layer_lr_scale < 1.0:
        ...
        def consume_verified(
            ...
            if not eligible:
                ...
                if config.execution_rule == "max_margin":
                    ...
                    chosen = min(
                        eligible,
                        key=lambda index: results[index].execution_cost,
                    )
                if config.archive_rule == "all_queries":
