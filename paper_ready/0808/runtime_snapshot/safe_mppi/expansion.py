...
class ExpansionConfig:
    ...
    def validate(self) -> None:
        ...
        if self.replay_batch_sampler == "mode_gamma_stratified":
            ...
            if self.batch_size < 4 * len(self.gammas):
                raise ValueError(
        ...
        if (
            ...
            > self.parallel_episodes * self.max_retry_batches * (
                2 if self.sample_update_mode is not None else 1
            )
        ):
            raise ValueError(
        ...
        if self.sample_update_mode is not None:
            ...
            if any(
                not isinstance(mode, int) or mode < 0 or mode > 3
                for mode in self.sample_update_mode
            ):
                raise ValueError("sample_update_mode entries must lie in [0,3]")
...
def _mode_gamma_stratified_batches(
    ...
    *,
    ...
    if missing or unexpected:
        raise ValueError(
            "mode_gamma_stratified replay requires all and only 4 x gamma "
...
def run_safe_expansion(
    ...
    *,
    ...
    ``ExpansionConfig.head_only_update``.
    """
    config.validate()
