...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="continue the deterministic episode/seed sequence from this index",
    )
    parser.add_argument(
        "--gammas",
        default=None,
        help="optional comma-separated subset of task-config gamma values",
    )
    ...
    configured_gammas = [float(value) for value in config.data.gammas]
    gammas = (
        [float(value) for value in args.gammas.split(",")]
        if args.gammas is not None else configured_gammas
    )
    unknown_gammas = [
    ...
    if unknown_gammas:
        parser.error(f"gammas are not in the task config: {unknown_gammas}")
