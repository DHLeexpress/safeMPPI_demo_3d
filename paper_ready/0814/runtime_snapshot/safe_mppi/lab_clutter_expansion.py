...
class LabClutterSphereExpansionTask:
    ...
    def successful_trajectory_quality(self, executed_states) -> dict[str, float]:
        """Knot-level admission diagnostics for committed clutter trajectories."""
        if not executed_states:
            raise ValueError("trajectory quality requires executed states")
        positions = np.vstack([
            np.asarray(self.env.start, dtype=float)[:3],
            *[np.asarray(state["x"], dtype=float)[:3] for state in executed_states],
        ])
        goal = np.asarray(self.env.goal, dtype=float)
        goal_distances = np.linalg.norm(positions - goal[None, :], axis=1)
        minimum_progress = float(np.min(goal_distances[:-1] - goal_distances[1:]))
        ...
        tortuosity = path_length / max(displacement, 1.0e-12)

        terminal_start = int(np.argmax(goal_distances <= 0.6))
        if not np.any(goal_distances <= 0.6):
            terminal_start = max(0, len(positions) - 2)
        terminal = positions[terminal_start:]
        cumulative = np.concatenate([[0.0], np.cumsum(
            np.linalg.norm(np.diff(terminal, axis=0), axis=1)
        )])
        if cumulative[-1] > 0.0:
            samples = np.arange(0.0, cumulative[-1] + 1.0e-12, 0.05)
            if samples[-1] < cumulative[-1]:
                samples = np.append(samples, cumulative[-1])
            resampled = np.column_stack([
                np.interp(samples, cumulative, terminal[:, axis])
                for axis in range(3)
            ])
        else:
            resampled = terminal[:1]
        directions = np.diff(resampled, axis=0)
        norms = np.linalg.norm(directions, axis=1)
        directions = directions[norms > 1.0e-9]
        if len(directions) < 2:
            terminal_turn = 0.0
        else:
            ...
            terminal_turn = float(np.sum(np.arccos(np.clip(
                np.sum(directions[:-1] * directions[1:], axis=1), -1.0, 1.0,
            ))))
        spheres = self.scene_spec.validate(self.env, executed_states[0]["spheres"])
        clearance = self._environment(spheres).obstacle_clearance(positions)
        return {
            "minimum_knot_goal_progress_m": minimum_progress,
            "tortuosity": float(tortuosity),
            "terminal_turn_rad_5cm": terminal_turn,
            "min_obstacle_surface_clearance_m": float(np.min(clearance)),
        }
