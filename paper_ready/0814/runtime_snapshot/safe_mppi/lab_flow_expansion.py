...
from .execution_costs import obstacle_conditioned_speed_cost
...
class LabFlowExpansionTask:
    ...
    def _execution_cost(
        ...
        if self.execution_obstacle_speed_weight > 0.0:
            cost += obstacle_conditioned_speed_cost(
                states,
                clearances,
                weight=self.execution_obstacle_speed_weight,
                clearance_target_m=self.execution_clearance_target_m,
    ...
    def successful_trajectory_min_clearance(self, executed_states) -> float:
        """Minimum modeled obstacle clearance on a terminal-success path."""
        if not executed_states:
            return -float("inf")
        clearance = self.env.obstacle_clearance(
            self._successful_trajectory_dense_path(executed_states)
        )
        finite = clearance[np.isfinite(clearance)]
        return float(finite.min()) if len(finite) else float("inf")

    def successful_trajectory_min_safety_margin(self, executed_states) -> float:
        """Minimum of obstacle clearance and physical task-space wall margin."""
        if not executed_states:
            return -float("inf")
        path = self._successful_trajectory_dense_path(executed_states)
        clearance = self.env.obstacle_clearance(path)
        finite = clearance[np.isfinite(clearance)]
        obstacle_margin = float(finite.min()) if len(finite) else float("inf")
        wall_margin = float(np.min(np.minimum(
            path - self.env.bounds[:, 0],
            self.env.bounds[:, 1] - path,
        )))
        return min(obstacle_margin, wall_margin)
