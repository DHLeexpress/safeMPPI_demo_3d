...
class ExpansionConfig:
    ...
    successful_trajectory_quality_gate: str = "none"
    successful_trajectory_min_goal_progress_m: float = 0.0
    successful_trajectory_max_tortuosity: float = 1.20
    successful_trajectory_max_terminal_turn_rad: float = math.pi / 2.0
    successful_trajectory_min_obstacle_clearance_m: float = 0.10
    successful_trajectory_min_taskspace_wall_clearance_m: float = 0.0
    ...
    def validate(self) -> None:
        ...
        if self.successful_trajectory_quality_gate not in {"none", "pair_atomic"}:
            raise ValueError(
                "successful_trajectory_quality_gate must be none or pair_atomic"
            )
        quality_values = (
            self.successful_trajectory_min_goal_progress_m,
            self.successful_trajectory_max_tortuosity,
            self.successful_trajectory_max_terminal_turn_rad,
            self.successful_trajectory_min_obstacle_clearance_m,
            self.successful_trajectory_min_taskspace_wall_clearance_m,
        )
        if not all(math.isfinite(float(value)) for value in quality_values):
            raise ValueError("successful trajectory quality thresholds must be finite")
        if (
            self.successful_trajectory_max_tortuosity < 1.0
            or self.successful_trajectory_max_terminal_turn_rad < 0.0
            or self.successful_trajectory_min_obstacle_clearance_m < 0.0
            or self.successful_trajectory_min_taskspace_wall_clearance_m < 0.0
        ):
            raise ValueError("successful trajectory quality thresholds are invalid")
        if self.successful_trajectory_quality_gate == "pair_atomic":
            ...
            if (
                ...
                raise ValueError(
                    "pair-atomic quality gating requires contiguous even exact "
                    "labels 0..Q-1"
                )
        replacement_enabled = (
...
def _pair_atomic_available_counts(
    counts: dict[int, int], requested_modes: Sequence[int],
) -> dict[int, int]:
    """Expose a mirror pair to quota admission only when both members exist."""
    available = {int(mode): int(counts.get(int(mode), 0)) for mode in requested_modes}
    for first in range(0, len(requested_modes), 2):
        pair = tuple(map(int, requested_modes[first:first + 2]))
        if len(pair) != 2 or pair[1] != (pair[0] ^ 1):
            raise ValueError("pair-atomic modes must be ordered mirror pairs")
        pair_count = min(available[pair[0]], available[pair[1]])
        available[pair[0]] = pair_count
        available[pair[1]] = pair_count
    return available


def _minimum_taskspace_wall_clearance(
    task: ExpansionTask, executed_states: Sequence[dict[str, Any]],
) -> float:
    """Return the robot-center signed margin to the closest task-space face."""
    env = getattr(task, "env", None)
    bounds = np.asarray(getattr(env, "bounds", None), dtype=float)
    start = np.asarray(getattr(env, "start", None), dtype=float)
    if bounds.shape != (3, 2) or start.shape[0] < 3:
        raise ValueError("task-space wall quality requires task.env bounds/start")
    if not executed_states:
        raise ValueError("task-space wall quality requires executed states")
    positions = np.vstack([
        start[:3],
        *[
            np.asarray(state["x"], dtype=float)[:3]
    ...
    return float(np.min(wall_clearance))
...
def _validate_bootstrap_records(
    records: Sequence[QueryRecord], config: ExpansionConfig,
) -> dict[str, Any]:
    """Validate a fresh-start replay/GP bootstrap without rewriting lineage."""
    rows = list(records)
    if not rows:
        raise ValueError("bootstrap query archive must contain QueryRecords")
    window_ids: set[str] = set()
    trajectory_cells: dict[str, tuple[float, int | None]] = {}
    for index, row in enumerate(rows):
        if not isinstance(row, QueryRecord):
            raise TypeError(f"bootstrap row {index} is not a QueryRecord")
        if row.round != 0 or not row.verification.valid or not row.replay_eligible:
            raise ValueError(
                "bootstrap rows must have round=0 and be valid/replay eligible"
            )
        if not any(math.isclose(row.gamma, gamma, abs_tol=1e-12, rel_tol=0.0)
                   for gamma in config.gammas):
            raise ValueError(f"bootstrap row has unconfigured gamma={row.gamma:g}")
        if not row.trajectory_id or not row.window_id:
            raise ValueError("bootstrap rows require trajectory_id and window_id")
        if row.window_id in window_ids:
            raise ValueError(f"duplicate bootstrap window_id: {row.window_id}")
        window_ids.add(row.window_id)
        cell = (float(row.gamma), row.sample_update_mode)
        previous = trajectory_cells.setdefault(row.trajectory_id, cell)
        if previous != cell:
            raise ValueError(
                f"bootstrap trajectory {row.trajectory_id} spans multiple strata"
            )
        for name in ("context", "candidate"):
            value = getattr(row, name)
            if not torch.is_tensor(value) or not torch.isfinite(value).all():
                raise ValueError(f"bootstrap row has invalid {name}")
        for name in ("flow_base", "loss_mask"):
            value = getattr(row, name)
            if value is not None and (
                not torch.is_tensor(value) or not torch.isfinite(value).all()
            ):
                raise ValueError(f"bootstrap row has invalid {name}")
        if row.flow_base is not None and row.flow_base.shape != row.candidate.shape:
            raise ValueError("bootstrap flow_base shape must match candidate")
        if row.loss_mask is not None and row.loss_mask.shape != row.candidate.shape:
            raise ValueError("bootstrap loss_mask shape must match candidate")
        if config.paired_noised_representation and row.flow_base is None:
            raise ValueError("paired bootstrap rows require flow_base")
        if config.replay_batch_sampler == "mode_gamma_stratified" and (
            row.sample_update_mode is None
        ):
            raise ValueError("stratified bootstrap rows require sample_update_mode")
    return {
        "row_count": len(rows),
        "trajectory_count": len(trajectory_cells),
        "identity_sha256": _record_identity_hash(rows),
    }
...
def _validate_resume_config(
    ...
    saved.setdefault("successful_trajectory_quality_gate", "none")
    saved.setdefault("successful_trajectory_min_goal_progress_m", 0.0)
    saved.setdefault("successful_trajectory_max_tortuosity", 1.20)
    saved.setdefault("successful_trajectory_max_terminal_turn_rad", math.pi / 2.0)
    saved.setdefault("successful_trajectory_min_obstacle_clearance_m", 0.10)
    saved.setdefault("successful_trajectory_min_taskspace_wall_clearance_m", 0.0)
...
def _run_safe_expansion_impl(
    ...
    *,
    ...
    bootstrap_records: Sequence[QueryRecord] | None = None,
    ...
    if config.successful_trajectory_quality_gate != "none":
        trajectory_quality_fn = getattr(
            task, "successful_trajectory_quality", None,
        )
        if not callable(trajectory_quality_fn):
            raise TypeError(
                "successful trajectory quality gating requires "
                "task.successful_trajectory_quality(executed_states)"
            )
    ...
    if resume_from is not None:
        if bootstrap_records is not None:
            raise ValueError("bootstrap query archive is allowed only on fresh start")
    ...
    if resume_state is None:
        if bootstrap_records is not None:
            bootstrap_provenance = _validate_bootstrap_records(
                bootstrap_records, config,
            )
            bootstrap_provenance["source_sha256"] = bootstrap_source_sha256
            archive = list(bootstrap_records)
            gp_evidence = list(bootstrap_records)
        _atomic_torch_save(
        ...
        bootstrap_provenance = resume_state.get("bootstrap_query_archive")
    ...
    for round_i in range(start_round, config.rounds + 1):
        ...
        def episode_quality_eligible(
            episode_record: dict[str, Any],
        ) -> bool:
            if config.successful_trajectory_quality_gate == "none":
                return True
            if "successful_trajectory_quality" not in episode_record:
                executed_states = [
                    step_record["state_after"]
                    for step_record in episode_record["executed_steps"]
                ]
                metrics = {
                    str(key): float(value)
                    for key, value in trajectory_quality_fn(executed_states).items()
                }
                metrics["min_taskspace_wall_clearance_m"] = float(
                    _minimum_taskspace_wall_clearance(task, executed_states)
                )
                required = {
                    "minimum_knot_goal_progress_m",
                    "tortuosity",
                    "terminal_turn_rad_5cm",
                    "min_obstacle_surface_clearance_m",
                    "min_taskspace_wall_clearance_m",
                }
                if set(metrics) != required or not all(
                    math.isfinite(value) for value in metrics.values()
                ):
                    raise ValueError(
                        "successful_trajectory_quality returned invalid metrics"
                    )
                eligible = (
                    metrics["minimum_knot_goal_progress_m"]
                    > config.successful_trajectory_min_goal_progress_m
                    and metrics["tortuosity"]
                    <= config.successful_trajectory_max_tortuosity
                    and metrics["terminal_turn_rad_5cm"]
                    <= config.successful_trajectory_max_terminal_turn_rad
                    and metrics["min_obstacle_surface_clearance_m"]
                    >= config.successful_trajectory_min_obstacle_clearance_m
                    and (
                        config.successful_trajectory_min_taskspace_wall_clearance_m
                        <= 0.0
                        or metrics["min_taskspace_wall_clearance_m"]
                        >= config.successful_trajectory_min_taskspace_wall_clearance_m
                    )
                )
                episode_record["successful_trajectory_quality"] = metrics
                episode_record["successful_trajectory_quality_eligible"] = bool(
                    eligible
                )
            return bool(episode_record["successful_trajectory_quality_eligible"])
        ...
        def missing_sample_update_modes(
            ...
            for episode_record in gamma_episodes:
                if not (
                    ...
                    and episode_quality_eligible(episode_record)
                ):
                    continue
            ...
            if config.successful_trajectory_quality_gate == "pair_atomic":
                available = _pair_atomic_available_counts(
                    available, requested_modes or (),
                )
            ...
            if not active:
                ...
                for gamma in map(float, config.gammas):
                    ...
                    commit_capable_successes = sum(
                        ...
                        and episode_quality_eligible(episode)
                        for episode in gamma_episodes
                    )
        ...
        if config.archive_rule == "successful_executed_windows":
            ...
            for gamma in config.gammas:
                ...
                if config.successful_trajectory_quality_gate == "pair_atomic":
                    available_modes = {
                        episode_sample_update_mode(episode)
                    ...
                    complete_modes = {
                        ...
                        if mode is not None
                        and (mode ^ 1) in available_modes
                    }
                    successful = [
                        episode for episode in successful
                        if episode_sample_update_mode(episode) in complete_modes
                    ]
                ...
                detail: dict[str, Any] = {
                    ...
                    "quality_gate": config.successful_trajectory_quality_gate,
                    "quality_eligible_episode_count": len(successful),
                    "success_episode_quality": {
                        str(int(episode["episode"])): episode.get(
                            "successful_trajectory_quality"
                        )
                        for episode in successful
                    },
        ...
        resume_payload = {
            ...
            "bootstrap_query_archive": bootstrap_provenance,
