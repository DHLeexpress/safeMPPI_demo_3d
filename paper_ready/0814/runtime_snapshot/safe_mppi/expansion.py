...
_ANGULAR8_SAMPLE_UPDATE_MODE = tuple(range(8))
_RESUME_MUTABLE_CONFIG_FIELDS = frozenset({
    # These affect only future rollout gathering or future archive admission.
    # Keeping them mutable is intentional: a committed boundary can change
    # its next-round sampling/admission design without discarding the learned
    # model, Adam momentum, RNG, replay archive, GP buffer, or history.
    ...
    "future_sampling_seed",
    "successful_trajectory_selector",
    "successful_trajectory_pool_multiplier",
    # These two fields form one future archive-admission contract.  Changing
    # them on an exact boundary preserves model/Adam/RNG/replay/GP/history,
    # while allowing a later round to admit a different number of newly
    # gathered terminal-success trajectories.  ExpansionConfig.validate()
    # still requires the mode list length to equal the trajectory quota.
    "successful_trajectories_per_gamma", "sample_update_mode",
...
class ExpansionConfig:
    ...
    execution_step_margin_mode: str = "linear"
    execution_cost_band_fraction: float = 0.0
    execution_cost_band_margin: str = "step"
    ...
    successful_trajectory_pool_multiplier: int = 1
    ...
    seed: int = 0
    future_sampling_seed: int | None = None
    ...
    def validate(self) -> None:
        ...
        if self.replay_batch_sampler == "mode_gamma_stratified":
            ...
            mode_count = len(set(self.sample_update_mode))
            if self.batch_size < mode_count * len(self.gammas):
                raise ValueError(
                    "mode_gamma_stratified batch_size must cover all "
                    f"{mode_count} x gamma strata"
                )
        ...
        if self.future_sampling_seed is not None and self.future_sampling_seed < 0:
            raise ValueError("future_sampling_seed must be nonnegative")
        ...
        if (
            ...
            and self.execution_rule not in {
                "min_cost", "exponential_cost", "quadratic_cost",
            }
        ):
            raise ValueError(
                "execution_step_margin_weight requires a cost-ranked "
                "execution rule"
            )
        if self.execution_step_margin_mode not in {"linear", "normalized"}:
            raise ValueError(
                "execution_step_margin_mode must be linear or normalized"
            )
        if (
        ...
        if self.execution_cost_band_margin not in {"step", "full"}:
            raise ValueError(
                "execution_cost_band_margin must be step or full"
            )
        if (
            ...
            raise ValueError(
                "execution_cost_band_fraction requires a cost-ranked "
                "execution rule"
            )
        if (
            self.execution_cost_band_fraction > 0.0
            ...
            raise ValueError(
                "execution cost-band and weighted margin hybrids are "
                "mutually exclusive"
            )
        ...
        if self.successful_trajectory_selector not in {
                ...
                "max_min_clearance", "max_min_safety_margin"}:
            raise ValueError(
                "successful_trajectory_selector must be lowest_episode_id, "
                "random_success, max_above_fraction, max_mean_z, "
                "max_min_clearance, or max_min_safety_margin"
            )
        if self.successful_trajectories_per_gamma < 1:
            raise ValueError(
                "successful_trajectories_per_gamma must be positive"
            )
        if self.successful_trajectory_pool_multiplier < 1:
            raise ValueError(
                "successful_trajectory_pool_multiplier must be positive"
            )
        cohort_multiplier = (
            2
        ...
        if (
            ...
            * self.successful_trajectory_pool_multiplier
        ...
        if self.sample_update_mode is not None:
            ...
            angular8 = (
                self.sample_update_mode == _ANGULAR8_SAMPLE_UPDATE_MODE
            )
            if any(
                not isinstance(mode, int) or mode < 0
                or (mode > 3 and not angular8)
                for mode in self.sample_update_mode
            ):
                raise ValueError(
                    "sample_update_mode entries must lie in [0,3], or equal "
                    "0,1,2,3,4,5,6,7 for the angular8 quota"
                )
        ...
        if self.retry_exhaustion_policy not in {"abort", "resample_scene"}:
            raise ValueError(
                "retry_exhaustion_policy must be abort or resample_scene"
            )
        if (
...
class ExpansionTask(Protocol):
    ...
    def successful_trajectory_submode(
        self, executed_states: Sequence[Any]
    ) -> int | None: ...
...
def _mode_gamma_stratified_batches(
    ...
    *,
    ...
    if missing or unexpected:
        raise ValueError(
            "mode_gamma_stratified replay requires all configured mode x "
            "gamma "
...
def _update(policy: ExpansionPolicy, task: ExpansionTask,
    ...
    if cfg.replay_batch_sampler == "mode_gamma_stratified":
        ...
        batches, replay_diagnostics = _mode_gamma_stratified_batches(
            ...
            modes=tuple(sorted(set(cfg.sample_update_mode or ()))),
        )
...
def _validate_resume_config(
    ...
    # Backward compatibility for exact snapshots committed before the
    # future-only sampling stream was introduced.  The default preserves the
    # historical behavior byte-for-byte.
    saved = dict(saved)
    saved.setdefault("future_sampling_seed", None)
    saved.setdefault("successful_trajectory_pool_multiplier", 1)
    ...
    archive_quota_changed = (
        saved["successful_trajectories_per_gamma"]
        != current_values["successful_trajectories_per_gamma"]
    )
    archive_modes_changed = (
        saved["sample_update_mode"] != current_values["sample_update_mode"]
    )
    if archive_quota_changed or archive_modes_changed:
        # This narrow exception supports balanced q12<->q24 admission from an
        # exact boundary.  The pool multiplier is independently future-only
        # and resume-mutable, so the number gathered in the next round may
        # differ from the historical round.  Both contracts must nevertheless
        # remain balanced over the same four route modes; this must not
        # silently turn four route modes into angular8 (or another classifier
        # contract) during a continuation.
        if not (archive_quota_changed and archive_modes_changed):
            raise ValueError(
                "successful_trajectories_per_gamma and sample_update_mode "
                "must change together on resume"
            )
        saved_modes = saved["sample_update_mode"]
        current_modes = current_values["sample_update_mode"]
        if saved_modes is None or current_modes is None:
            raise ValueError(
                "resume archive quota changes require explicit four-mode "
                "sample_update_mode contracts"
            )

        def effective_mode_counts(
            modes: Sequence[int], multiplier: int,
        ) -> dict[int, int]:
            counts = {mode: 0 for mode in range(4)}
            for mode in modes:
                if mode not in counts:
                    raise ValueError(
                        "resume archive quota changes only support the same "
                        "four route modes"
                    )
                counts[mode] += int(multiplier)
            return counts

        before_counts = effective_mode_counts(
            saved_modes, saved["successful_trajectory_pool_multiplier"],
        )
        after_counts = effective_mode_counts(
            current_modes,
            current_values["successful_trajectory_pool_multiplier"],
        )
        if (
            len(set(before_counts.values())) != 1
            or len(set(after_counts.values())) != 1
        ):
            raise ValueError(
                "resume archive quota changes require balanced per-mode "
                "gathered rollout quotas before and after the boundary"
            )
    changed = {
...
def _run_safe_expansion_impl(
    ...
    *,
    ...
    round_committed_callback: Callable[[dict[str, Any]], None] | None = None,
    ...
    sampling_seed = (
        int(config.seed)
        if config.future_sampling_seed is None
        else int(config.future_sampling_seed)
    )
    sample_update_mode_fn = None
    if config.sample_update_mode is not None:
        mode_function_name = (
            "successful_trajectory_submode"
            if config.sample_update_mode == _ANGULAR8_SAMPLE_UPDATE_MODE
            else "successful_trajectory_mode"
        )
        sample_update_mode_fn = getattr(
            task, mode_function_name, None,
        )
        if not callable(sample_update_mode_fn):
            raise TypeError(
                "sample_update_mode requires "
                f"task.{mode_function_name}(executed_states)"
            )
    ...
    for round_i in range(start_round, config.rounds + 1):
        ...
        gather_complete_s_by_gamma: dict[float, float] = {}
                ...
                for replica in range(config.parallel_episodes):
                    ...
                    if config.archive_rule == "successful_executed_windows":
                        reset_seed = _counter_seed(
                            sampling_seed, "reset", round_i, retry_batch, replica
                        )
                    else:
                        reset_seed = sampling_seed + 10000 * round_i + episode
                    episodes.append({
        ...
        def episode_sample_update_mode(
            ...
            if "sample_update_mode" not in episode_record:
                ...
                if mode is not None:
                    ...
                    max_mode = (
                        7
                        if config.sample_update_mode
                        == _ANGULAR8_SAMPLE_UPDATE_MODE
                        else 3
                    )
                    if mode < 0 or mode > max_mode:
                        raise ValueError(
                            "successful_trajectory_mode must return None or "
                            f"an integer in [0,{max_mode}]"
                        )
                episode_record["sample_update_mode"] = mode
            return episode_record["sample_update_mode"]

        def requested_sample_update_pool_modes() -> tuple[int, ...] | None:
            if config.sample_update_mode is None:
                return None
            return tuple(
                mode
                for mode in config.sample_update_mode
                for _ in range(config.successful_trajectory_pool_multiplier)
            )

        sample_update_pool_modes = requested_sample_update_pool_modes()

        def successful_pool_complete(
            gamma_episodes: Sequence[dict[str, Any]],
        ) -> bool:
            if sample_update_pool_modes is not None:
                return not missing_sample_update_modes(gamma_episodes)
            commit_capable_successes = sum(
                episode["status"] == "SUCCESS"
                and bool(episode.get("commit_capable", False))
                for episode in gamma_episodes
            )
            return commit_capable_successes >= (
                config.successful_trajectories_per_gamma
                * config.successful_trajectory_pool_multiplier
            )

        def missing_sample_update_modes(
            gamma_episodes: Sequence[dict[str, Any]],
        ) -> list[int]:
            if sample_update_pool_modes is None:
                return []
            ...
            missing = []
            for requested_mode in sample_update_pool_modes:
            ...
            if not eligible:
                ...
                if config.execution_rule == "max_margin":
                    ...
                    if config.execution_cost_band_fraction > 0.0:
                        ...
                        minimum_cost = min(eligible_costs.values())
                        cost_span = max(eligible_costs.values()) - minimum_cost
                        threshold = (
                            minimum_cost
                            + config.execution_cost_band_fraction * cost_span
                        )
                        shortlist = [
                            index for index in eligible
                            if eligible_costs[index] <= threshold
                        ]
                        if config.execution_cost_band_margin == "step":
                            if any(
                                results[index].step_margin is None
                                for index in shortlist
                            ):
                                raise ValueError(
                                    "step cost-band requires verifier "
                                    "step_margin values"
                                )
                            margin_key = lambda index: float(  # noqa: E731
                                results[index].step_margin
                            )
                        else:
                            margin_key = lambda index: float(  # noqa: E731
                        ...
                        chosen = max(
                            shortlist,
                            key=lambda index: (
                                margin_key(index), -eligible_costs[index],
                            ),
                        )
                        execution_key = eligible_costs.__getitem__
                    elif (
                        config.execution_step_margin_weight > 0.0
                        and config.execution_step_margin_mode == "normalized"
                    ):
                        eligible_costs = torch.as_tensor(
                            [results[index].execution_cost for index in eligible],
                            dtype=torch.float64,
                        )
                        eligible_margins = torch.as_tensor(
                            [
                        ...
                        cost_span = (
                            eligible_costs.max() - eligible_costs.min()
                        ).clamp_min(1.0e-12)
                        margin_span = (
                            eligible_margins.max() - eligible_margins.min()
                        ).clamp_min(1.0e-12)
                        normalized_score = (
                            (eligible_costs - eligible_costs.min()) / cost_span
                            - config.execution_step_margin_weight
                            * (eligible_margins - eligible_margins.min())
                            / margin_span
                        )
                        score_by_index = {
                            index: float(normalized_score[local])
                            for local, index in enumerate(eligible)
                        }
                        execution_key = score_by_index.__getitem__
                    else:
            ...
            if not active:
                ...
                for gamma in map(float, config.gammas):
                    ...
                    if quota_complete:
                        ...
                        gather_complete_s_by_gamma.setdefault(
                            gamma, time.perf_counter() - gather_started,
                        )
        ...
        if (
            ...
            if config.sample_update_mode is not None:
                ...
                for gamma in sorted(exhausted_retry_gammas):
                    ...
                    parts.append(f'gamma={gamma:g} need mode "{needed}"')
            ...
            if config.retry_exhaustion_policy == "resample_scene":
                raise RuntimeError(
                    ...
                    f"obtaining {config.successful_trajectories_per_gamma} "
        ...
        if config.archive_rule == "successful_executed_windows":
            ...
            for gamma in config.gammas:
                ...
                safety_selector_active = (
                    config.successful_trajectory_selector
                    in {"max_min_clearance", "max_min_safety_margin"}
                )
                min_clearance_fn = (
                    getattr(
                        task, "successful_trajectory_min_clearance", None
                    )
                    if safety_selector_active else None
                )
                min_safety_margin_fn = (
                    getattr(
                        task, "successful_trajectory_min_safety_margin", None
                    )
                    if safety_selector_active else None
                )
                above_fractions: dict[int, float | None] = {}
                mean_z_values: dict[int, float | None] = {}
                min_clearance_values: dict[int, float | None] = {}
                min_safety_margin_values: dict[int, float | None] = {}
                for successful_episode in successful:
                    ...
                    if callable(min_clearance_fn):
                        min_clearance = float(min_clearance_fn([
                            step_record["state_after"]
                            for step_record in successful_episode["executed_steps"]
                        ]))
                        if not math.isfinite(min_clearance):
                            raise ValueError(
                                "successful trajectory min clearance must be finite"
                            )
                        min_clearance_values[episode_id] = min_clearance
                    else:
                        min_clearance_values[episode_id] = None
                    if callable(min_safety_margin_fn):
                        min_safety_margin = float(min_safety_margin_fn([
                            step_record["state_after"]
                            for step_record in successful_episode["executed_steps"]
                        ]))
                        if not math.isfinite(min_safety_margin):
                            raise ValueError(
                                "successful trajectory min safety margin must "
                                "be finite"
                            )
                        min_safety_margin_values[episode_id] = min_safety_margin
                    else:
                        min_safety_margin_values[episode_id] = None
                sample_update_modes = {
                ...
                if (
                    ...
                    if not callable(min_clearance_fn):
                        raise TypeError(
                            "max_min_clearance requires task."
                            "successful_trajectory_min_clearance"
                        )
                    successful.sort(key=lambda episode: (
                        -float(min_clearance_values[int(episode["episode"])]),
                        int(episode["episode"]),
                    ))
                elif (
                    successful
                    and config.successful_trajectory_selector
                    == "max_min_safety_margin"
                ):
                    if not callable(min_safety_margin_fn):
                        raise TypeError(
                            "max_min_safety_margin requires task."
                            "successful_trajectory_min_safety_margin"
                        )
                    successful.sort(key=lambda episode: (
                        -float(min_safety_margin_values[
                            int(episode["episode"])
                        ]),
                        int(episode["episode"]),
                    ))
                    ...
                    successful.sort(key=lambda episode: (
                        _counter_seed(
                            sampling_seed, "successful_trajectory_selector",
                            round_i, f"{gamma:.9g}",
                            int(episode["episode"]),
                        ),
                        int(episode["episode"]),
                ...
                if config.sample_update_mode is not None:
                    ...
                    for requested_mode in config.sample_update_mode:
                ...
                if config.sample_update_mode is not None:
                    detail.update({
                        "sample_update_mode": list(config.sample_update_mode),
                ...
                if (
                    config.successful_trajectory_pool_multiplier != 1
                    or config.successful_trajectory_selector
                    in {"max_min_clearance", "max_min_safety_margin"}
                ):
                    detail.update({
                        "success_episode_min_clearance_m": {
                            str(episode_id): min_clearance_values[episode_id]
                            for episode_id in sorted(min_clearance_values)
                        },
                        "success_episode_min_safety_margin_m": {
                            str(episode_id): min_safety_margin_values[episode_id]
                            for episode_id in sorted(min_safety_margin_values)
                        },
                        "pool_multiplier": (
                            config.successful_trajectory_pool_multiplier
                        ),
                        "pool_trajectory_count": (
                            config.successful_trajectories_per_gamma
                            * config.successful_trajectory_pool_multiplier
                        ),
                    })
                ...
                for rank, committed_episode in enumerate(
                    ...
                    if (
                        ...
                        trajectory_detail["min_clearance_m"] = (
                            min_clearance_values[episode_id]
                        )
                        trajectory_detail["min_safety_margin_m"] = (
                            min_safety_margin_values[episode_id]
                        )
        ...
        row = {
            ...
            "gather_complete_s_by_gamma": {
                f"{gamma:.9g}": gather_complete_s_by_gamma.get(gamma)
                for gamma in map(float, config.gammas)
            },
        ...
        if round_committed_callback is not None:
            round_committed_callback(row)
    manifest = {
        ...
        "semantics": {
            ...
            "execution": (
                "positive-per-knot-x verifier positives ranked by "
                f"{config.execution_rule}; nominal one-step H_P is "
                + (
                    "also used by the declared execution hybrid"
                    if (
                        config.execution_step_margin_weight > 0.0
                        or (
                            config.execution_cost_band_fraction > 0.0
                            and config.execution_cost_band_margin == "step"
                        )
                    )
            ...
            "successful_executed_windows": (
                ...
                if config.archive_rule != "successful_executed_windows"
                    ...
                    "after gathering "
                    f"{config.successful_trajectories_per_gamma * config.successful_trajectory_pool_multiplier} "
                    "commit-capable terminal SUCCESS trajectories per gamma, "
                    "choose "
                    f"{config.successful_trajectories_per_gamma} distinct terminal "
                    "SUCCESS trajectories per gamma, ranked by "
                    "successful_trajectory_selector"
                    + (
                        f", gathering {config.successful_trajectory_pool_multiplier}x "
                        f"the exact mode quota {list(config.sample_update_mode)} "
                        "and committing the original exact mode quota"
                        if config.sample_update_mode is not None else ""
                    )
            ...
            "episode_retry": (
                ...
                if config.archive_rule != "successful_executed_windows"
                    ...
                    + (
                        f" satisfying {config.successful_trajectory_pool_multiplier}x "
                        f"the exact mode quota {list(config.sample_update_mode)}; "
                        "ranking then commits only the original quota and later "
                        "successes in already-full pool modes are not admitted"
                        if config.sample_update_mode is not None else ""
                    )
    ...
    *,
    ...
    with _OrderedVerifier(task, config.verifier_workers) as verifier:
        return _run_safe_expansion_impl(
            ...
            round_committed_callback=round_committed_callback,
