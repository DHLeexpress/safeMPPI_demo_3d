"""Raw fixed-temperature evaluation for Minhyuk-frame flow expansion."""
...
def _checkpoint_policy(
    ...
    return policy.to(torch.device(device)).eval()
...
@torch.no_grad()
def _probe_rows(policy, config, gammas, samples, seed, device="cpu"):
    ...
    for gamma in gammas:
        for probe_index, state6 in enumerate(probes):
            ...
            context = task.context(state, float(gamma)).to(device)
...
def _plot_curves(
    per_round_rows, summaries, gammas, output,
    *, x_label="Expansion round",
):
    del summaries
...
def _draw_mode_sectors(axis, center_uv, reach, label=True):
    """Draw the below/above/left/right decision boundaries in a head-on plot.

    A rollout's mode comes from theta = atan2(vertical, transverse) of its
    first forward crossing of the sphere's axial plane, measured from the
    sphere centre, binned as below [-135,-45), left [-45,45),
    above [45,135), right otherwise.  Those bin edges are the four rays at
    +/-45 and +/-135 degrees drawn here, so a trajectory's colour and the
    sector it crosses in always agree.
    """
    for degrees in (45.0, 135.0, 225.0, 315.0):
        angle = np.radians(degrees)
        axis.plot(
            [center_uv[0], center_uv[0] + reach * np.cos(angle)],
            [center_uv[1], center_uv[1] + reach * np.sin(angle)],
            color="#5f6368", lw=0.9, ls=(0, (6, 4)), alpha=0.75, zorder=2,
        )
    if not label:
        return
    offset = 0.62 * reach
    for name, degrees in (
        ("left", 0.0), ("above", 90.0), ("right", 180.0), ("below", 270.0),
    ):
        angle = np.radians(degrees)
        axis.text(
            center_uv[0] + offset * np.cos(angle),
            center_uv[1] + offset * np.sin(angle),
            name, color=MODE_COLORS[name], ha="center", va="center",
            fontsize=13, alpha=0.85, zorder=3,
        )


def _plot_gallery(
    env, per_round_rows, rounds, gammas, output, view="side",
    *, stage_label="round",
):
    """Per-round/per-gamma raw rollout gallery.

    ``view='side'`` keeps the canonical along-path projection (start-goal axis
    vs vertical).  ``view='head_on'`` looks straight down the start-goal
    vector, i.e. transverse vs vertical, which is the plane in which the
    below/above/left/right crossing modes separate; unlike the success-only
    crossing fan this keeps failures and colours by route mode.
    """
    if view not in {"side", "head_on"}:
        raise ValueError("gallery view must be 'side' or 'head_on'")
    _paper_rcparams()
    ...
    horizontal = 0 if view == "side" else 1
    head_on_limits = None
    if view == "head_on":
        spread = [np.zeros((1, 2))]
        for round_i in rounds:
            for row in per_round_rows[round_i]:
                ...
                spread.append(local[:, 1:3])
        points = np.concatenate(spread)
        centre = np.asarray(
            [float(center_local[1]), float(center_local[2])]
        )
        half = float(np.abs(points - centre).max())
        half = max(half, 3.0 * radius) * 1.08
        head_on_limits = (
    ...
    for row_index, round_i in enumerate(rounds):
        for column_index, gamma in enumerate(gammas):
            ...
            axis.fill(
                center_local[horizontal] + radius * np.cos(theta),
            ...
            axis.scatter(
                goal_local[horizontal], goal_local[2], marker="*",
            ...
            if view == "head_on":
                _draw_mode_sectors(
                    ...
                    (float(center_local[1]), float(center_local[2])),
                    reach=3.0 * radius,
                    label=(row_index == 0 and column_index == 0),
                )
            axis.grid(alpha=0.2)
            ...
            if head_on_limits is not None:
                axis.set_xlim(*head_on_limits[0])
                axis.set_ylim(*head_on_limits[1])
            else:
                extent = (corners - env.start[:3]) @ frame[:, horizontal]
                axis.set_xlim(
                    float(extent.min()) - 0.1,
                    float(extent.max()) + 0.1,
                )
                axis.set_ylim(
            ...
            if column_index == 0:
                axis.set_ylabel(f"{stage_label} {round_i}\nvertical [m]")
            if row_index == len(rounds) - 1:
                axis.set_xlabel(
                    "start-goal axis [m]" if view == "side"
                    else "transverse [m]"
                )
    handles = [
        Line2D([0], [0], color=MODE_COLORS[mode], lw=2.4, label=mode)
        for mode in ROUTE_MODES
    ]
    handles.append(
        Line2D([0], [0], color="#5f6368", lw=1.0, ls=(0, (6, 4)),
               label="mode boundary (+/-45 deg)")
    )
    fig.legend(
    ...
    if view == "head_on":
        fig.suptitle(
            "head-on view down the start-goal axis", y=0.91, fontsize=16,
        )
    fig.tight_layout(rect=(0, 0, 1, 0.86 if view == "head_on" else 0.93))
    fig.savefig(output, dpi=200, bbox_inches="tight")
...
def _plot_sr_coverage(
    per_round_rows, gammas, output, *, x_label="Expansion round",
):
    rounds = sorted(per_round_rows)
    colors = {
        gamma: plt.get_cmap("plasma")(
...
def _validate_replay_provenance(manifest, committed_cells):
    ...
    if config["replay_selector"] != "uniform":
        raise ValueError(
            "replay-pool provenance visualization requires uniform replay"
        )
    exact_adam_membership = config.get("inner_steps") is None
...
def _plot_mechanism_multiview(
    ...
    exact_adam_membership,
):
    """Static W-round replay provenance for one gamma."""
    frame = start_goal_frame(env)
    ...
    for row_index, round_i in enumerate(rounds):
        ...
        provenance = (
            "exact Adam rows" if exact_adam_membership
            else "eligible replay pool (Adam row-capped)"
        )
        side.set_title(
...
def _compose_summary(output_dir):
    ...
    # --gallery-view decides which gallery exists; the summary shows whichever
    # was rendered rather than assuming the side view is always present.
    gallery = (
        ("raw_gallery.png", "Raw temperature-1 gallery")
        if (output_dir / "raw_gallery.png").is_file()
        else ("raw_gallery_headon.png", "Raw temperature-1 gallery (head-on)")
    )
    paths = (
        ("raw_crossing_fan.png", "Raw crossing fan"),
        gallery,
        ("raw_curves.png", "Raw temperature-1 metrics"),
        ("mechanism_multiview.png", "Replay evidence provenance"),
    )
...
def _mechanism_video(
    ...
    *,
    ...
    """Reference-style gathering movie with replay provenance."""
    plt.rcParams.update({
        "font.family": "serif",
        "mathtext.fontset": "cm",
        "axes.titlesize": 11.5,
    ...
    with writer.saving(fig, str(output), dpi=135):
        for round_i in sorted(set(int(value) for value in rounds)):
            ...
            committed_mode_ids = current_cell.detail.get(
                "committed_sample_update_modes"
            )
            mode_names = ("below", "above", "left", "right")
            committed_mode_by_episode = (
                {
                    int(episode_id): int(mode)
                    for episode_id, mode in zip(
                        current_cell.committed_episode_ids,
                        committed_mode_ids,
                    )
                }
                if committed_mode_ids is not None else {}
            )
            committed_mode_counts = [
                sum(int(mode) == mode_id for mode in committed_mode_ids)
                for mode_id in range(4)
            ] if committed_mode_ids is not None else None
            ...
            for episode in current_cell.committed_episodes:
                mode_id = committed_mode_by_episode.get(episode.episode)
                _draw_episode(
                    side, head, episode, env, frame,
                    color=(
                        MODE_COLORS[mode_names[mode_id]]
                        if mode_id is not None else COMMITTED_COLOR
                    ),
                    linewidth=3.0,
                )
            positions = _local_path(
            ...
            manifest_row = manifest["rounds"][round_i - 1]
            provenance = (
                "exact Adam input" if exact_adam_membership
                else "eligible replay pool; Adam rows capped"
            )
            side.set_title(
                rf"round {round_i} | $\gamma={gamma:g}$ | {provenance}"
                "\n"
                ...
                + (
                    "\ncommitted routes "
                    f"b{committed_mode_counts[0]}/a{committed_mode_counts[1]}"
                    f"/l{committed_mode_counts[2]}/r{committed_mode_counts[3]}"
                    if committed_mode_counts is not None else ""
                ),
                fontsize=10.5,
            )
            head.set_title("head-on", fontsize=10.5)
            ...
            committed_legend = (
                tuple(
                    Line2D(
                        [0], [0], color=MODE_COLORS[name], lw=3.0,
                        label=(
                            f"committed {name} "
                            f"({committed_mode_counts[mode_id]})"
                        ),
                    )
            ...
            summary_legend = committed_legend + (
                Line2D(
                    [0], [0], marker="o", color="none",
                    markerfacecolor=COMMITTED_WINDOW_COLOR,
                    markeredgecolor="white", markersize=7,
                ...
                Line2D(
                    ...
                    label="failed gathering attempt (not replay eligible)",
                ),
            )
            side.legend(
...
def _evaluation_output_path(args) -> Path:
    requested = getattr(args, "evaluation_output", None)
    return Path(requested) if requested is not None else args.expansion / "eval"
...
def evaluate_lab_expansion(args, config, pretrain_manifest, manifest):
    ...
    diagnostic_stages = any(
        token in str(manifest.get("kind", "")).lower()
        for token in ("memorization", "interpolation", "posttrain")
    )
    x_label = "Checkpoint stage" if diagnostic_stages else "Expansion round"
    stage_label = "stage" if diagnostic_stages else "round"
    ...
    requested_rounds = getattr(args, "evaluation_rounds", None)
    if requested_rounds is None:
        ...
        eval_rounds = sorted(set(int(value) for value in requested_rounds))
        invalid = [
            round_i for round_i in eval_rounds
            if round_i < 0 or round_i > total_rounds
        ]
        if invalid:
            raise ValueError(
                "evaluation rounds must lie in "
                f"[0,{total_rounds}], got {invalid}"
            )
        missing = [
            round_i for round_i in eval_rounds
            if not (
        ...
        if missing:
            raise FileNotFoundError(
                "requested evaluation checkpoints are not published: "
                + ", ".join(str(round_i) for round_i in missing)
            )
    per_round_rows, per_round_probes, summaries = {}, {}, {}
    for progress_index, round_i in enumerate(eval_rounds, start=1):
        policy = _checkpoint_policy(
            args.pretrain_dir, args.expansion, round_i, device=device,
        )
        if getattr(args, "flow_nfe", None) is not None:
            policy.flow.nfe = int(args.flow_nfe)
        rows = _raw_rows(
            policy, config, gammas, int(args.episodes), int(args.seed),
            device=device, sampling_temperature=sampling_temperature,
        )
        probes = _probe_rows(
            policy,
            config,
    ...
    output = _evaluation_output_path(args)
    ...
    (output / "raw_eval.json").write_text(json.dumps({
        "status": (
            "LAB_RAW_TEMPERATURE1_EVALUATION_COMPLETE"
            if sampling_temperature == 1.0
            else "LAB_RAW_FIXED_TEMPERATURE_EVALUATION_COMPLETE"
        ),
        "sampling_temperature": sampling_temperature,
        "flow_nfe_override": getattr(args, "flow_nfe", None),
    ...
    if getattr(args, "save_raw_trajectories", False):
        torch.save(per_round_rows, output / "raw_trajectories.pt")
    ...
    requested_gallery = getattr(args, "gallery_rounds", None)
    if requested_gallery:
        gallery_rounds = sorted(set(map(int, requested_gallery)))
        missing = [r for r in gallery_rounds if r not in per_round_rows]
        if missing:
            raise ValueError(
                f"--gallery-rounds {missing} were not evaluated; evaluated "
                f"rounds are {sorted(per_round_rows)} (widen --stride or use "
                "--evaluation-rounds)"
            )
    else:
    ...
    if getattr(args, "gallery_view", "side") in {"side", "both"}:
        _plot_gallery(
            env,
            per_round_rows,
            gallery_rounds,
    ...
    if getattr(args, "gallery_view", "side") in {"head_on", "both"}:
        _plot_gallery(
            env,
            per_round_rows,
            gallery_rounds,
            gammas,
            output / "raw_gallery_headon.png",
            view="head_on",
            stage_label=stage_label,
        )
    _plot_crossing_fan(
    ...
    if not args.screening_only:
        ...
        exact_adam_membership = _validate_replay_provenance(
            manifest, committed_cells,
        )
        policy = _checkpoint_policy(
            args.pretrain_dir, args.expansion, total_rounds, device=device,
        )
        if getattr(args, "flow_nfe", None) is not None:
            policy.flow.nfe = int(args.flow_nfe)
        task = LabFlowExpansionTask(
        ...
        _plot_mechanism_multiview(
            ...
            exact_adam_membership=exact_adam_membership,
        )
        _mechanism_video(
            env,
            task,
