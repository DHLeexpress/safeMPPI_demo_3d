...
def angular_submode(theta: float | None) -> int | None:
    """Split each below/above/left/right quadrant into two 45-degree bins.

    IDs are paired by the existing route order: below ``0,1``, above
    ``2,3``, left ``4,5``, and right ``6,7``.  The original quadrant
    boundaries are unchanged; each quadrant is split at its cardinal axis.
    """
    if theta is None:
        return None
    angle = (float(np.degrees(theta)) + 180.0) % 360.0 - 180.0
    if -135.0 <= angle < -90.0:
        return 0
    if -90.0 <= angle < -45.0:
        return 1
