...
def main():
    ...
    parser.add_argument(
        "--expansion-manifest", type=Path,
        help=(
            "optional read-only manifest override for evaluating a published "
            "checkpoint before the live expansion publishes its final manifest"
        ),
    )
    parser.add_argument(
        "--lab-task-config", type=Path,
        help=(
            "optional read-only lab task-config override for a causal "
            "evaluation; checkpoints and expansion artifacts remain unchanged"
        ),
    )
    parser.add_argument(
        "--evaluation-output",
    ...
    parser.add_argument(
        "--flow-nfe", type=int,
        help="override flow Euler integration steps for a sampling-only audit",
    )
    parser.add_argument("--stride", type=int, default=1)
    parser.add_argument(
        "--evaluation-rounds", type=int, nargs="+",
        help=(
            "evaluate only these published checkpoints instead of deriving "
            "the checkpoint set from --stride; useful for nonblocking "
            "coverage audits while expansion is still running"
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "retain the fixed-bank raw states, controls and dense integration "
            "steps in raw_trajectories.pt for trajectory-level diagnosis"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "RESEARCH: relax the OOB survival floor to this z during "
            "evaluation rollouts. The planepack/visual context still reads "
            "the original taskspace bounds, so policy conditioning is "
            "unchanged; only where rollouts are allowed to fly changes"
        ),
    )
    ...
    if args.geofence_floor_z is not None:
        from safe_mppi.environment import TaskEnvironment as _TaskEnv
        _relaxed_floor = float(args.geofence_floor_z)
        _original_inside = _TaskEnv.inside_taskspace

        def _relaxed_inside(self, points, tolerance=1e-7):
            pts = np.asarray(points, float).reshape(-1, 3)
            lower = np.asarray(self.bounds[:, 0], float).copy()
            if _relaxed_floor < lower[2]:
                lower[2] = _relaxed_floor
            return np.all(
                (pts >= lower - tolerance)
                & (pts <= np.asarray(self.bounds[:, 1], float) + tolerance),
                axis=1,
            )

        _TaskEnv.inside_taskspace = _relaxed_inside
        print(
            f"[geofence] eval survival floor relaxed to {_relaxed_floor} "
            "(context keeps original bounds)",
            flush=True,
        )
    if args.helios:
        return run_evaluation_on_helios(
            args, sys.argv[1:], ROOT,
            script_name="research_evaluate_ball_expansion.py",
        )
    if (
    ...
    if (
        ...
        early_manifest_path = (
            ...
            else args.expansion / "manifest.json"
        )
        manifest = json.loads(early_manifest_path.read_text())
        task_config_path = (
            args.lab_task_config
            if args.lab_task_config is not None
            else args.expansion / "task_config_resolved.json"
        )
    ...
    if args.evaluation_rounds is None:
        eval_rounds = sorted({
            0, *range(0, total_rounds + 1, args.stride), total_rounds,
        })
    else:
        eval_rounds = sorted(set(args.evaluation_rounds))
        invalid = [
        ...
        missing = [
            round_i for round_i in eval_rounds
            if not (args.expansion / f"checkpoint_{round_i:03d}.pt").is_file()
        ]
        if missing:
            raise FileNotFoundError(
    ...
    if args.save_raw_trajectories:
        torch.save(per_round_rows, eval_dir / "raw_trajectories.pt")
