...
def _sampling_payload(spec: PathFocusedClutterSpec) -> dict:
    ...
    elif spec.distribution == PATH_FOCUSED_DOUBLE_HOURGLASS_DISTRIBUTION:
        payload.update({
            "transverse_distribution": (
                "piecewise_linear_symmetric_uniform"
            ),
            "transverse_halfwidth_control_points": (
                spec.transverse_halfwidth_control_points
            ),
            "sphere_z_distribution": (
                "symmetric_longitudinal_scaled_uniform"
            ),
            "sphere_z_plane_m": spec.sphere_z_plane_m,
            "sphere_vertical_halfwidth_scale": (
                spec.sphere_vertical_halfwidth_scale
            ),
        })
...
def main() -> None:
    ...
    cylinder_bounds = np.asarray(cylinder_config.taskspace.bounds, np.float64)
    sphere_bounds = np.asarray(sphere_config.taskspace.bounds, np.float64)
    cylinder_start = np.asarray(cylinder_config.taskspace.start[:3], np.float64)
    cylinder_goal = np.asarray(cylinder_config.taskspace.goal, np.float64)
    sphere_start = np.asarray(sphere_config.taskspace.start[:3], np.float64)
    sphere_goal = np.asarray(sphere_config.taskspace.goal, np.float64)
    cylinders = [
    ...
    payload = {
        ...
        "taskspace_bounds": cylinder_bounds.tolist(),
        "start": cylinder_start.tolist(),
        "goal": cylinder_goal.tolist(),
        "sphere_taskspace_bounds": sphere_bounds.tolist(),
        "sphere_start": sphere_start.tolist(),
        "sphere_goal": sphere_goal.tolist(),
        "sampling": sampling,
