"""Run Stage-2 Safe Flow Expansion in randomized lab sphere scenes.

This entry point is intentionally separate from ``run_ball_expansion.py``. It
accepts only the visual lab policy and either a fixed three-sphere task or a
path-focused variable-count sphere task. The authoritative recipe uses
successful executed windows, paired noised representations, and current-model
GP re-embedding; incompatible defaults fail before any output is written.
"""
...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "required randomized lab sphere task config"
        ),
    )
    parser.add_argument(
        "--device",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "for the T128 checkpoint N=1 updates flow.trunk.4 (48->32) and "
            "flow.head (32->30)"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "what to do when a gamma exhausts --max-retry-batches without "
            "enough commit-capable successes: abort restores the historical "
            "hard failure, while resample_scene (default) keeps launching "
            "whole-episode batches on freshly sampled scenes so the round "
            "can still commit"
        ),
    )
    parser.add_argument(
        ...
        help=(
            "absolute per-gamma whole-episode batch budget (including the "
            "first --max-retry-batches) when "
            "--retry-exhaustion-policy=resample_scene; exceeding it fails "
            "the round"
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "min_cost-only override of the native SafeMPPI quadratic "
            "soft-clearance weight; omitted preserves the task config"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "min_cost-only override of the native taskspace exponential "
            "weight; pass 0 to remove J_bound from execution ranking while "
            "preserving verifier and rollout OOB checks"
        ),
    )
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "clutter min_cost-only Gaussian penalty on nominal states near "
            "the z_bias_plane (default 0.9 m), discouraging purely lateral "
            "left/right routes; 0 disables"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "box so a broad heavy in-plane penalty still separates "
            "off-plane candidates"
        ),
    )
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "clutter min_cost-only linear penalty on nominal height above "
            "the z_bias_plane, softly preferring below routes over above; "
            "0 disables"
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "recipe-compatibility flag for lab tasks: the verifier always "
            "requires only the dense first executed segment to remain inside "
            "the configured lab geofence; the legacy [0,3] x [1.5,2.5] "
            "single-ball corridor is never imported"
        ),
    )
    parser.add_argument(
        "--verifier-mode",
    ...
    if args.helios:
        return run_expansion_on_helios(
            args,
            sys.argv[1:],
            ROOT,
            script_name="run_sphere_clutter_expansion.py",
        )
    required_contract = {
        "--archive-rule": (
    ...
    if mismatches:
        parser.error(
            "sphere-clutter expansion recipe is incomplete; this commonly means "
    ...
    if (
        ...
        and (
            ...
            or args.execution_plane_penalty_weight != 0.0
    ...
    if lab_profile:
        try:
            ...
            if clutter_profile:
                ...
                if (
                    minimum_count < 1
                    or int(scene_spec.max_count) < minimum_count
                ):
                    raise ValueError(
                        "sphere-clutter config has an invalid obstacle-count "
                        "range"
                    )
    ...
    def round_progress(round_row):
        ...
        print(format_round_summary(round_row), flush=True)
    ...
    if not lab_profile:
        manifest["ball_verifier_corridor"] = {
            ...
            "z": [1.5, 2.5],
            "contract": (
                "corridor exit along the dense first executed segment makes "
                "a query invalid; the unexecuted H-1 tail is not corridor-gated"
            ),
        }
    target_geometry = {
        "above_wedge": {
    ...
    if lab_profile:
        ...
        manifest["task_profile"] = (
            (
                ...
                if task.scene_schema == LAB_CLUTTER_SCENE_SCHEMA
                    ...
                    "minhyuk_lab_perpendicular_triangle_sphere_"
                    "visual_expansion"
                    if task.scene_schema
                    == PERPENDICULAR_TRIANGLE_SCENE_SCHEMA
                    else (
        ...
        manifest["lab_execution_cost"] = {
            ...
            "native_soft_clearance": {
                ...
                "formula": (
                    "weight * sum_h max(target_m-clearance_h, 0)^2"
                ),
