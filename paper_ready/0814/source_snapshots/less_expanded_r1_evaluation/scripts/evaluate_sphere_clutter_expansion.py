"""Evaluate randomized sphere-clutter expansion checkpoints.

Metrics use disjoint raw temperature-1 rollouts. A separately fixed randomized
sphere scene supports visual comparison across checkpoints, while the
mechanism video renders the exact per-episode spheres and acquisition events
used during training. Single-ball above/below counters are deliberately not
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            "fixed randomized scene-bank size per gamma/checkpoint; Stage-2 "
            "defaults to the requested 50-scene true evaluation"
        ),
    )
    ...
    parser.add_argument(
        "--fixed-scene-mode",
        choices=("auto", "sampled", "triangle_base", "bowling_123"),
        default="auto",
        help=(
            "auto uses the unperturbed tight triangle for the triangle law "
            "and a disjoint sampled scene otherwise; bowling_123 uses the "
            "fixed six-sphere projected 8-route coverage diagnostic"
        ),
    )
    ...
    if args.helios:
        return run_evaluation_on_helios(
            ...
            sys.argv[1:],
            ROOT,
            script_name="evaluate_sphere_clutter_expansion.py",
        )
    if (
    ...
    if not (
        ...
        parser.error(
            "sphere-clutter evaluation requires a visual lab pretrained policy"
        )
    ...
    if not is_lab_clutter_evaluation_manifest(manifest):
        parser.error(
            "expansion manifest is not a randomized-sphere clutter run"
        )
    if not task_config_path.is_file():
        raise FileNotFoundError(
            "sphere-clutter expansion requires task_config_resolved.json"
        )
    ...
    if minimum_count < 1 or int(scene_spec.max_count) < minimum_count:
        parser.error("sphere-clutter config has an invalid obstacle-count range")
    evaluate_lab_clutter_expansion(
        args, task_config, pretrain_manifest, manifest,
    )
    return

    # The copied single-ball implementation remains below for provenance, but
    # this dedicated entry point returns above and can never enter it.
    if (
        pretrain_manifest.get("kind")
        == "lab raw-command reference-flow pretraining"
        or str(pretrain_manifest.get("context_schema", "")).startswith("lab_")
