#!/usr/bin/env python3
"""Stage and run the fixed-scene PRE2 gamma-diversity diagnostic."""
...
def main() -> None:
    ...
    parser.add_argument("--helios-gpu", type=int, default=3)
    args, extra = parser.parse_known_args()
    if args.helios_gpu not in {1, 3}:
        parser.error("gamma-diversity diagnostic permits only GPU 1 or 3")
    ...
    with _ssh_master(HELIOS_HOST) as ssh:
        ...
        command, status_file, log_file = _remote_detached_command(
            remote_source,
            remote_output,
            "preflight_multisphere_gamma_diversity.py",
            remote_args,
            args.helios_gpu,
            share_gpu=True,
            job_token=job_token,
    ...
    if not (local_output / "diversity.pt").is_file() or not (
        local_output / "summary.json"
    ).is_file():
        raise RuntimeError(
            "Helios gamma-diversity run completed without final artifacts"
        )
    (local_output / ".helios_remote.json").write_text(json.dumps({
        "status": "HELIOS_REMOTE_GAMMA_DIVERSITY_COMPLETE",
        "host": HELIOS_HOST,
