"""Run isolated PRE2 exact multi-pair multi-sphere expansion.
...
def _resolve_multipair_quota(
    trajectories_per_gamma: int,
    parallel_episodes: int,
) -> tuple[int, tuple[int, ...]]:
    """Return pair count and exact labels for an even trajectory quota."""
    quota = int(trajectories_per_gamma)
    parallel = int(parallel_episodes)
    if quota < 2 or quota % 2:
        raise ValueError(
            "multi-pair --successful-trajectories-per-gamma must be an "
            "even trajectory count of at least two"
        )
    if parallel % quota:
        raise ValueError(
            "multi-pair --parallel-episodes must be a multiple of "
            "--successful-trajectories-per-gamma so every batch has "
            "equal attempts for all pair-slot/member labels"
        )
    return quota // 2, tuple(range(quota))


def _opposite_multipair_label(label: int) -> int:
    """Return the opposite member label in the same exact scene-pair slot."""
    value = int(label)
    if value < 0:
        raise ValueError("multi-pair label must be nonnegative")
    return value ^ 1


def _retained_committed_pair_replacements(
    events: list[dict], completed_round: int,
) -> list[dict]:
    """Keep cumulative replacement provenance through a resume boundary."""
    if int(completed_round) < 0:
        raise ValueError("completed_round must be nonnegative")
    retained = []
    for event in events:
        round_i = int(event["round"])
        if round_i <= int(completed_round):
            retained.append(dict(event))
    return retained


def _discard_replaced_pair_mirror_state(
    trace_cache: dict,
    context_offers: dict,
    acquisition_queue: deque,
    episode_rows: dict,
    *,
    round_i: int,
    gamma: float,
    ...
    """Remove every mirror runtime artifact tied to one obsolete pair."""
    labels = (2 * int(pair_slot), 2 * int(pair_slot) + 1)
    invalidated_trace_count = 0
    for label in labels:
        key = (int(round_i), float(gamma), label, int(old_version))
        invalidated_trace_count += int(trace_cache.pop(key, None) is not None)

    def is_replaced_metadata(metadata: dict | None) -> bool:
        return bool(metadata) and (
            int(metadata.get("round", -1)) == int(round_i)
            and float(metadata.get("gamma", float("nan"))) == float(gamma)
            and int(metadata.get("pair_slot", -1)) == int(pair_slot)
            and int(metadata.get("paired_scene_version", -1))
            == int(old_version)
        )

    stale_offer_keys = [
        key for key, metadata in context_offers.items()
        if is_replaced_metadata(metadata)
    ]
    for key in stale_offer_keys:
        context_offers.pop(key, None)

    retained_queue = deque(
        metadata for metadata in acquisition_queue
        if not is_replaced_metadata(metadata)
    )
    invalidated_queue_count = len(acquisition_queue) - len(retained_queue)
    acquisition_queue.clear()
    acquisition_queue.extend(retained_queue)

    stale_episode_keys = []
    for key, rows in episode_rows.items():
        if rows and is_replaced_metadata({
            "round": rows[0].get("round"),
            "gamma": rows[0].get("gamma"),
            "pair_slot": rows[0].get("paired_scene_pair_slot"),
            "paired_scene_version": rows[0].get("paired_scene_version"),
        }):
            stale_episode_keys.append(key)
    for key in stale_episode_keys:
        episode_rows.pop(key, None)

    return {
        "invalidated_mirror_trace_count": invalidated_trace_count,
        "invalidated_mirror_offer_count": len(stale_offer_keys),
        "invalidated_mirror_acquisition_queue_count": (
            invalidated_queue_count
        ),
        "invalidated_mirror_episode_buffer_count": len(stale_episode_keys),
    }
...
def _terminal_success_trace_from_compact_rows(
    ...
    *,
    ...
    identity_fields = (
        ...
        "paired_scene_pair_slot", "paired_scene_label", "paired_scene_version",
    ...
    pair_slot = identity["paired_scene_pair_slot"]
    label = identity["paired_scene_label"]
    version = identity["paired_scene_version"]
    if (
        pair_slot is None
        or label is None
        or version is None
        or int(pair_slot) < 0
        or int(version) < 0
        or int(label) != 2 * int(pair_slot) + int(member)
    ):
        raise ValueError(
            "terminal-success mirror source has inconsistent pair-slot label"
        )
    ...
    return {
        ...
        "paired_pair_slot": int(pair_slot),
        "paired_label": int(label),
        "paired_version": int(version),
...
def main():
    ...
    parser.add_argument(
        ...
        default="none",
        help=(
            "for each round and gamma, freeze Q/2 distinct randomized scene "
            "pairs and their exact 180-degree rotations about the 3-D "
            "start-goal axis; --successful-trajectories-per-gamma=Q counts "
            "actual trajectories and must be even"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--paired-scene-replace-after-retry-batches",
        type=int,
        default=8,
        help=(
            "at or after this completed retry-batch index, replace a pair "
            "slot when either member still lacks a commit-capable terminal "
            "success; both member quotas reset"
        ),
    )
    parser.add_argument(
        "--paired-scene-max-replacements-per-slot",
        type=int,
        default=1,
        help=(
            "hard per-round/gamma/pair-slot replacement cap; default one "
            "prevents unbounded scene churn"
        ),
    )
    parser.add_argument(
        "--paired-scene-replace-interval-batches",
        type=int,
        default=1,
        help=(
            "minimum completed whole retry batches between successive "
            "replacements of the same pair slot; one preserves the legacy "
            "every-batch behavior"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        choices=("abort", "resample_scene", "commit_available"),
        default="abort",
        help=(
            "RESEARCH: on retry exhaustion, keep drawing fresh episode "
            "batches (resample_scene), or commit the quota-eligible successes "
            "already found (commit_available) instead of aborting the round"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--successful-trajectories-per-round",
        type=int,
        help=(
            "optional exact total across all gamma cells; for mirrored pairs "
            "the even per-gamma quotas rotate deterministically by round"
        ),
    )
    parser.add_argument(
        "--sample-update-mode",
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "min_cost-only bounded-regret hybrid: among candidates within "
            "this fraction of the selected-B cost span above the minimum, "
            "execute the one with maximum first-step nominal H_P margin"
        ),
    )
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "exponential obstacle weight on mean_h "
            "exp((target-clearance_h)/temperature); the isolated "
            "multi-sphere task adds it to min_cost execution ranking"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "multi-sphere min_cost-only weight on mean_h "
            "sigmoid((clearance_target-clearance_h)/temperature) * ||v_h||^2"
        ),
    )
    parser.add_argument(
    ...
    if args.paired_scene_rotation != "start_goal_axis_180":
        parser.error(
            "isolated multi-pair entrypoint requires "
            "--paired-scene-rotation start_goal_axis_180"
        )
    if args.fixed_scene_layout != "none":
        parser.error(
            "isolated multi-pair entrypoint does not support a fixed scene"
        )
    if args.paired_scene_replace_after_retry_batches < 1:
        parser.error(
            "--paired-scene-replace-after-retry-batches must be positive"
        )
    if args.paired_scene_max_replacements_per_slot < 1:
        parser.error(
            "--paired-scene-max-replacements-per-slot must be positive"
        )
    if args.paired_scene_replace_interval_batches < 1:
        parser.error(
            "--paired-scene-replace-interval-batches must be positive"
        )
    multipair_pair_count = 0
    multipair_labels: tuple[int, ...] = ()
    if args.paired_scene_rotation != "none":
        try:
            multipair_pair_count, multipair_labels = (
                _resolve_multipair_quota(
    ...
    if args.successful_trajectories_per_round is not None:
        if args.successful_trajectories_per_round < 8:
            parser.error(
                "--successful-trajectories-per-round must allow one pair for "
                "each of four gammas"
            )
        if args.successful_trajectories_per_round % 2:
            parser.error(
                "--successful-trajectories-per-round must be even for exact "
                "mirror pairs"
            )
    ...
    if (
        ...
        parser.error(
            "--execution-obstacle-speed-weight must be finite and nonnegative"
        )
    if (
    ...
    if (
        args.execution_cost_band_fraction > 0.0
        and args.execution_rule != "min_cost"
    ):
        parser.error(
            "--execution-cost-band-fraction requires --execution-rule min_cost"
        )
    if (
        args.execution_cost_band_fraction > 0.0
        and args.execution_step_margin_weight > 0.0
    ):
        parser.error(
            "--execution-cost-band-fraction and "
            "--execution-step-margin-weight are mutually exclusive"
        )
    if (
        args.execution_obstacle_speed_weight > 0.0
        and args.execution_rule != "min_cost"
    ):
        parser.error(
            "--execution-obstacle-speed-weight requires --execution-rule min_cost"
        )
    if (
    ...
    if (
        ...
        and args.execution_rule not in {"exponential_cost", "min_cost"}
    ):
        parser.error(
            "--execution-clearance-exp-weight requires "
            "--execution-rule exponential_cost, or min_cost for the "
            "isolated multi-sphere task"
        )
    if (
        ...
        try:
            ...
            if clutter_profile:
                ...
                task = LabClutterPre2MultiPairExpansionTask(
                    ...
                    paired_scene_pair_count=(
                        multipair_pair_count
                        if multipair_pair_count > 0 else 1
                    ),
                    paired_scene_max_replacements_per_slot=(
                        args.paired_scene_max_replacements_per_slot
                    ),
    ...
    if (
        ...
        parser.error(
            "--execution-clearance-exp-weight with --execution-rule min_cost "
            "is supported only by the isolated multi-sphere lab task"
        )
    if (
        args.execution_obstacle_speed_weight > 0.0
        and not (lab_profile and clutter_profile)
        ...
        parser.error(
            "--execution-obstacle-speed-weight is supported only by the "
            "isolated multi-sphere lab task"
        )
    if (
        args.execution_plane_escape_weight != 0.0
        and not (lab_profile and not clutter_profile)
    ...
    if paired_scene_quota:
        ...
        # Labels 2*slot and 2*slot+1 identify one original/axis-180
        # trajectory from each distinct deterministic scene-pair slot.
        sample_update_mode = multipair_labels
    ...
    config = ExpansionConfig(
        ...
        execution_cost_band_fraction=args.execution_cost_band_fraction,
        ...
        successful_trajectories_per_round=(
            args.successful_trajectories_per_round
        ),
        ...
        paired_scene_replace_after_retry_batches=(
            args.paired_scene_replace_after_retry_batches
        ),
        paired_scene_replace_interval_batches=(
            args.paired_scene_replace_interval_batches
        ),
        paired_scene_max_replacements_per_slot=(
            args.paired_scene_max_replacements_per_slot
        ),
    ...
    pair_replacement_log_path = (
        args.output / "paired_scene_replacements.json"
    )
    previous_pair_replacement_events: list[dict] = []
    if args.resume_from is not None and pair_replacement_log_path.is_file():
        previous_replacement_log = json.loads(
            pair_replacement_log_path.read_text()
        )
        previous_pair_replacement_events = list(
            previous_replacement_log.get("events", [])
        )
    ...
    if args.resume_from is not None:
        ...
        if (
            resume_completed_round > 0
            and not pair_replacement_log_path.is_file()
        ):
            raise FileNotFoundError(
                "exact multi-pair resume is missing replacement provenance: "
                f"{pair_replacement_log_path}"
            )
        previous_pair_replacement_events = (
            _retained_committed_pair_replacements(
                previous_pair_replacement_events,
                resume_completed_round,
            )
        )
        ...
        if clutter_profile:
            ...
            if previous_manifest_path.is_file():
                ...
                observed_scene_versions: dict[
                    tuple[int, float, int], set[int]
                ] = {}
                for event in events:
                    key = (
                        int(event["round"]), float(event["gamma"]),
                        int(event["paired_scene_label"]),
                    )
                    observed_scene_hashes.setdefault(key, set()).add(
                        str(event["scene_hash"])
                    )
                    observed_scene_versions.setdefault(key, set()).add(
                        int(event["paired_scene_version"])
                    )
                ...
                for round_index in range(1, resume_completed_round + 1):
                    ...
                    # Materialize the complete version-0 registry first, then
                    # replay the bounded replacements in their recorded order.
                    # This preserves cross-gamma distinctness and exact hashes.
                    for gamma in config.gammas:
                        scheduled_quota = _successful_trajectory_quota_for(
                            config, round_index, float(gamma),
                        )
                        for label in range(
                            scheduled_quota
                        ):
                            task.reset(float(gamma), label, 0)
                    for replacement in previous_pair_replacement_events:
                        if int(replacement["round"]) != round_index:
                            continue
                        reconstructed = task.replace_paired_scene_slot(
                            float(replacement["gamma"]),
                            int(replacement["pair_slot"]),
                        )
                        for name in (
                            "old_version", "new_version",
                            "old_source_scene_hash",
                            "old_rotated_scene_hash",
                            "new_source_scene_hash",
                            "new_rotated_scene_hash",
                        ):
                            if reconstructed[name] != replacement[name]:
                                raise ValueError(
                                    "paired replacement reconstruction "
                                    f"mismatch for {name}: "
                    ...
                    for gamma in config.gammas:
                        ...
                        for label in range(
                            ...
                            key = (round_index, float(gamma), label)
                            hashes = observed_scene_hashes.get(key, set())
                            versions = observed_scene_versions.get(key, set())
                            state = task.reset(float(gamma), label, 0)
                            if (
                                not hashes
                                and not versions
                                and args.retry_exhaustion_policy
                                == "commit_available"
                            ):
                                # commit_available intentionally leaves some
                                # scheduled labels without an executed success.
                                # Their frozen geometry is still determined by
                                # the round/gamma/label seed registry and never
                                # entered replay.  Materialize it so a later
                                # round resumes exactly; observed labels below
                                # remain hash-verified against committed events.
                                task.scene_ledger[-1][
                                    "resume_reconstructed_unobserved_pair"
                                ] = True
                                continue
                            if len(hashes) != 1 or len(versions) != 1:
                                raise ValueError(
                                    "cannot reconstruct multi-pair scene "
                                    "registry for round/gamma/label "
                                    f"{key}: hashes={hashes}, versions={versions}"
                                )
                            if (
                                str(state["scene_hash"]) != next(iter(hashes))
                                or int(state["paired_scene_version"])
                                != next(iter(versions))
                            ):
                                raise ValueError(
    ...
    def callback(event):
        ...
        if clutter_profile:
            ...
            compact_event.update({
                ...
                "paired_scene_pair_slot": state_before.get(
                    "paired_scene_pair_slot"
                ),
                "paired_scene_label_base": state_before.get(
                    "paired_scene_label_base"
                ),
                "paired_scene_label": state_before.get(
                    "paired_scene_label"
                ),
                "paired_scene_version": state_before.get(
    ...
    pair_replacement_stats = {
        "contract": (
            "at/after the configured completed retry-batch index, replace "
            "only an incomplete current-version slot whose original or "
            "axis-180 member lacks a commit-capable terminal success; "
            "discard both labels and "
            "recollect both on the fresh version"
        ),
        "replace_after_retry_batches": int(
            args.paired_scene_replace_after_retry_batches
        ),
        "max_replacements_per_slot": int(
            args.paired_scene_max_replacements_per_slot
        ),
        "replace_interval_batches": int(
            args.paired_scene_replace_interval_batches
        ),
        "events": list(previous_pair_replacement_events),
    }

    def _write_pair_replacement_stats() -> None:
        args.output.mkdir(parents=True, exist_ok=True)
        temporary = pair_replacement_log_path.with_name(
            f".{pair_replacement_log_path.name}.tmp"
        )
        temporary.write_text(
            json.dumps(pair_replacement_stats, indent=2) + "\n"
        )
        temporary.replace(pair_replacement_log_path)

    def _paired_label_name(label: int) -> str:
        pair_slot, member = divmod(int(label), 2)
        member_name = "original" if member == 0 else "axis_180"
        return f"pair_{pair_slot}:{member_name}"
    ...
    def retry_progress(retry_row):
        ...
        fa_stats["retry_progress"].append({
            ...
            "paired_scene_replacements": list(
                retry_row.get("paired_scene_replacements", [])
            ),
        })
        _write_fa_stats()
        if paired_scene_quota:
            needed_names = [
                _paired_label_name(int(mode))
                for mode in retry_row["needed_modes"]
            ]
            pair_counts = " ".join(
                f"p{slot}=o{unguided[2 * slot]}/"
                f"a{unguided[2 * slot + 1]}"
                for slot in range(multipair_pair_count)
            )
            print(
    ...
    from safe_mppi import expansion_multisphere_multipair as _expansion_mod

    mirror_trace_cache: dict[tuple[int, float, int, int], list[dict]] = {}
    ...
    mirror_stats = {
        ...
        "contract": (
            ...
            "(round,gamma,pair_slot,label,scene_version); on retries for the "
            "still-missing "
            "opposite label in the same pair slot, replace global K-1 with "
            "the exact R=2uu^T-I "
        ...
        "needed_mode_gate": (
            ...
            "the full multi-pair retry cohort is preserved, so this may "
            "waste verified offers for a label filled since the last report, "
    ...
    def _mirror_round_bucket(round_i: int) -> dict:
        return mirror_stats["rounds"].setdefault(str(int(round_i)), {
            ...
            "max_alignment_position_error": 0.0,
            "max_alignment_velocity_error": 0.0,
            "by_gamma_pair_slot_target_label": {},
        })
    ...
    def _mirror_offer_bucket(metadata: dict) -> dict:
        ...
        name = (
            ...
            f"p{int(metadata['pair_slot'])}_"
            f"l{int(metadata['target_label'])}_"
            f"v{int(metadata['paired_scene_version'])}"
        )
        return bucket["by_gamma_pair_slot_target_label"].setdefault(name, {
    ...
    def _mirror_trace_key(
        round_i: int, gamma: float, label: int, version: int,
    ):
        return int(round_i), float(gamma), int(label), int(version)
    ...
    def _cache_terminal_success_trace(extracted: dict) -> None:
        ...
        pair_slot = int(extracted["paired_pair_slot"])
        label = int(extracted["paired_label"])
        version = int(extracted["paired_version"])
        key = _mirror_trace_key(round_i, gamma, label, version)
        if key in mirror_trace_cache:
            return
        trace = extracted["trace"]
        mirror_trace_cache[key] = trace
        trace_name = (
            f"r{int(round_i):03d}_g{float(gamma):.9g}_"
            f"p{pair_slot}_l{label}_v{version}"
        )
    ...
    def _paired_mirror_proposal_hook(
        *, episode, step, gamma, round_i, context, state_before,
        ...
        if int(episode.get("retry_batch", 0)) > 0:
            ...
            pair_slot = int(state_before.get("paired_scene_pair_slot", -1))
            target_label = int(state_before.get("paired_scene_label", -1))
            target_version = int(state_before.get("paired_scene_version", -1))
            ...
            if (
                pair_slot < 0
                or target_version < 0
                or target_label != 2 * pair_slot + target_member
            ):
                raise RuntimeError(
                    "paired mirror proposal target lost its pair-slot label"
                )
            source_member = 1 - target_member
            source_label = _opposite_multipair_label(target_label)
            if source_label != 2 * pair_slot + source_member:
                raise RuntimeError(
                    "paired mirror proposal crossed scene-pair slots"
                )
            source_key = _mirror_trace_key(
                round_i, gamma, source_label, target_version,
            )
            source_trace = mirror_trace_cache.get(source_key)
            progress_key = (int(round_i), float(gamma))
            needed_labels = retry_needed_modes.get(progress_key, [])
            if (
                target_label in needed_labels
                ...
                metadata = {
                    ...
                    "paired_scene_version": int(target_version),
    ...
    def pair_replacement_progress(replacement_row: dict) -> None:
        round_i = int(replacement_row["round"])
        gamma = float(replacement_row["gamma"])
        pair_slot = int(replacement_row["pair_slot"])
        old_version = int(replacement_row["old_version"])
        logged = dict(replacement_row)
        logged.update(_discard_replaced_pair_mirror_state(
            mirror_trace_cache,
            mirror_context_offers,
            mirror_acquisition_queue,
            mirror_episode_rows,
            round_i=round_i,
            gamma=gamma,
        ...
        pair_replacement_stats["events"].append(logged)
        _write_pair_replacement_stats()
        print(
            f"[paired-scene-replace] r{round_i} gamma {gamma:g} "
            f"retry {int(logged['trigger_completed_retry_batch'])} "
            f"p{pair_slot} v{old_version}->v{int(logged['new_version'])} "
            f"commit o{logged['trigger_commit_capable_success_counts'][0]}/"
            f"a{logged['trigger_commit_capable_success_counts'][1]} "
            f"(terminal {logged['trigger_terminal_success_counts'][0]}/"
            f"{logged['trigger_terminal_success_counts'][1]}); "
            f"discarded {int(logged['invalidated_terminal_success_count'])}; "
            f"hash {logged['old_source_scene_hash'][:8]}/"
            f"{logged['old_rotated_scene_hash'][:8]} -> "
            f"{logged['new_source_scene_hash'][:8]}/"
            f"{logged['new_rotated_scene_hash'][:8]}",
            flush=True,
        )
    ...
    def _committed_modes_line(round_row: dict) -> str:
        ...
        if clutter_profile:
            if paired_scene_quota:
                labels = [0] * effective_successful_trajectories_per_gamma
                for detail in round_row[
                    "successful_executed_commit_by_gamma"
                ].values():
                    for label in detail["committed_sample_update_modes"]:
                        labels[int(label)] += 1
                pair_summary = " ".join(
                    f"p{slot}=o{labels[2 * slot]}/"
                    f"a{labels[2 * slot + 1]}"
                    for slot in range(multipair_pair_count)
                )
                return f" paired {pair_summary}"
            return ""
        if sample_update_mode is not None:
    ...
    def round_progress(round_row):
        ...
        # The core publishes its committed resume snapshot immediately after
        # this callback. Persist the cumulative replacement ledger here so an
        # abrupt later-round interruption always has matching provenance.
        _write_pair_replacement_stats()
        show_progress(
            "Stage-1 expansion",
            int(round_row["round"]),
    ...
    try:
        manifest = run_safe_expansion(
            ...
            pair_replacement_callback=pair_replacement_progress,
        ...
        if not output_was_unsafe:
            ...
            failure = {
                ...
                "paired_scene_replacement_diagnostics": (
                    pair_replacement_stats
                ),
            }
            if args.event_log == "committed_success":
    ...
    if lab_profile:
        ...
        manifest["lab_execution_cost"] = {
            ...
            "bounded_regret_margin_hybrid": {
                "cost_band_fraction": float(
                    args.execution_cost_band_fraction
                ),
                "formula": (
                    "among selected-B candidates with J <= J_min + "
                    "fraction*(J_max-J_min), maximize first-step nominal "
                    "H_P margin; break ties by lower J"
                ),
                "margin_is_not_verifier_label": True,
                "scope": "execution ranking only",
            },
            ...
            "execution_obstacle_conditioned_speed": {
                ...
                "formula": (
                    "weight * mean_h sigmoid((target_m-clearance_h)/"
                    "temperature_m) * ||predicted_velocity_h||^2"
                ),
                "scope": (
                    "execution ranking only; bounded proximity gate; "
                    "verifier/acquisition/dynamics unchanged"
                ),
            },
            "execution_clearance_exponential": {
                ...
                "effective_weight": float(
                    args.execution_clearance_exp_weight
                    if (
                        args.execution_rule == "exponential_cost"
                        or (
                            args.execution_rule == "min_cost"
                            and clutter_profile
                        )
                ...
                "predicted_states": "H post-action plan knots",
                "applies_only_to": (
                    "multi-sphere min_cost execution ranking"
                    if args.execution_rule == "min_cost" and clutter_profile
                    else "single-sphere exponential_cost execution ranking"
                ),
                "scope": (
                    "H post-action plan knots; execution ranking only; "
                    "verifier/acquisition/rollout termination unchanged"
                ),
            },
        ...
        manifest["lab_coverage_objective"] = {
            ...
            "sample_update_mode_names": (
                {
                    str(label): _paired_label_name(label)
                    for label in range(
                        effective_successful_trajectories_per_gamma
                    )
                }
            ...
            "sample_update_mode_classifier": (
                (
                    "scene state metadata: paired_scene_label = "
                    "2*pair_slot + member (internal admission label, not a "
                    "route mode)"
                    if paired_scene_quota else
            ...
            "sample_update_retry_guidance": (
                (
                    ...
                    "launches equal attempts on every distinct frozen "
                    "original/axis_180 scene-pair slot; exactly one success "
                    "per pair-slot/member label is committed"
                    if paired_scene_quota else (
        ...
        if clutter_profile:
            ...
            manifest["lab_scene_rng_contract"] = {
                "reset": (
                    "Q/2 distinct deterministic frozen original/axis_180 "
                    "scene pairs per round and gamma; retries change rollout "
                    "latents only"
                    if paired_scene_quota else (
                ...
                "mode_labels_or_quota": (
                    "pair-slot/member admission labels 0..Q-1; no route labels"
            ...
            manifest["lab_paired_scene_quota"] = {
                ...
                "requested_trajectories_per_gamma_per_round": (
                    args.successful_trajectories_per_gamma
                    if paired_scene_quota else 0
                ),
                "requested_trajectories_per_round": (
                    args.successful_trajectories_per_round
                    if paired_scene_quota else None
                ),
                "scheduled_trajectories_by_round_gamma": (
                    {
                        str(round_i): {
                            f"{float(gamma):g}": (
                                _successful_trajectory_quota_for(
                                    config, round_i, float(gamma),
                                )
                            )
                            for gamma in config.gammas
                        }
                ...
                "pair_slot_count": (
                    multipair_pair_count if paired_scene_quota else 0
                ),
                "replace_after_retry_batches": int(
                    args.paired_scene_replace_after_retry_batches
                ),
                ...
                "replacement_semantics": (
                    "incomplete-pair commit-capable terminal-success trigger; "
                    "replacement "
                    "invalidates both labels, increments scene version, and "
                    "requires both members to be recollected"
                ),
                "replacement_ledger": list(
                    pair_replacement_stats["events"]
                ),
                "committed_trajectories_per_gamma_per_round": (
                    "see scheduled_trajectories_by_round_gamma"
                    if args.successful_trajectories_per_round is not None
                    else effective_successful_trajectories_per_gamma
                ),
                "pair_admission": (
                    "one original plus one axis_180 success for every distinct "
                    "frozen round/gamma/pair-slot geometry; label successes "
                    "accumulate across retry batches and surplus successes are "
                    "excluded from the exact final commit"
                    if paired_scene_quota else None
                ),
                "fixed_scene_layout": args.fixed_scene_layout,
    ...
    expected_trajectories = sum(
        _successful_trajectory_quota_for(config, round_i, float(gamma))
        for round_i in range(1, args.rounds + 1)
        for gamma in config.gammas
    )
    if (
        committed_trajectories != expected_trajectories
        and args.retry_exhaustion_policy != "commit_available"
    ):
        raise RuntimeError(
    ...
    manifest["paired_scene_replacement_diagnostics"] = (
        pair_replacement_stats
    )
