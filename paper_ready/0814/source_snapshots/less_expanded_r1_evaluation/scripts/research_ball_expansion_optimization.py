"""Run exact-quota ball expansion with Reserve-G-scale online optimization.

This is an intentionally independent copy of ``research_ball_expansion.py``.
It keeps the exact guided/unguided quota collector while defaulting the update
path to cumulative mode x gamma stratified trajectory replay, trunk-3 updates,
batch 256, and one exact 50k-step cosine schedule across all rounds.
"""
...
def main():
    ...
    parser.add_argument(
        "--sample-update-submodes",
        choices=("none", "angular8"),
        default="none",
        help=(
            "default none preserves the four-route quota exactly; angular8 "
            "replaces --sample-update-mode with one exact SUCCESS quota in "
            "each of eight 45-degree crossing sectors (two per existing "
            "below/above/left/right mode) and requires "
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "lambda in cost - lambda * first-step nominal H_P margin for "
            "min_cost/exponential_cost/quadratic_cost; zero preserves native "
            "cost ranking"
        ),
    )
    parser.add_argument(
        "--execution-step-margin-mode",
        choices=("linear", "normalized"),
        default="linear",
        help=(
            "linear uses the raw cost and margin scales; normalized rescales "
            "both across the current verifier-eligible B candidates before "
            "applying --execution-step-margin-weight"
        ),
    )
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "bounded-regret hybrid: shortlist verified candidates whose cost "
            "is within this fraction of the eligible B cost span above the "
            "minimum, then maximize the selected margin; zero disables"
        ),
    )
    parser.add_argument(
        ...
        default="step",
        help=(
            "margin maximized inside --execution-cost-band-fraction: first-"
            "step nominal H_P (step) or full-H verifier slack (full)"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="mean",
        help=(
            "single-sphere exponential_cost horizon aggregation; mean "
            "preserves the legacy score exactly, max uses the worst knot, "
            "and top3_mean averages the largest min(3,H) knot penalties"
        ),
    )
    parser.add_argument(
        ...
        type=float,
        default=0.20,
        help="clearance target in meters for optional obstacle terms",
    )
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "single-sphere cost-ranked weight on mean_h "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "execution-ranking override of the native SafeMPPI control "
            "energy weight; omitted preserves the task config"
        ),
    )
    parser.add_argument(
        "--execution-terminal-goal-weight",
        type=float,
        default=None,
        help=(
            "execution-ranking override of the terminal squared goal-distance "
            "weight; omitted preserves the task config"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "additive execution-ranking weight on terminal-knot clearance "
            "shortfall from exactly the x_max and y_min goal-side faces"
        ),
    )
    parser.add_argument(
        ...
        default=0.60,
        help=(
            "symmetric terminal-knot clearance target for the x_max and "
            "y_min goal-side faces"
        ),
    )
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "additive execution-ranking weight on the mean full-H "
            "hinge-exponential distance outside a goal-centered box; 0 "
    ...
    parser.add_argument(
        ...
        default=0.20,
        help=(
            "half extent in every coordinate of the virtual box centered at "
            "the goal; this never changes physical task-space bounds"
        ),
    )
    parser.add_argument(
        ...
        type=float,
        default=0.50,
        help="positive temperature of the goal-box hinge-exponential cost",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "measure the optional cylinder term to the finite start-goal "
            "segment, adding smooth spherical end caps instead of using an "
            "infinite axis"
        ),
    )
    parser.add_argument(
        "--execution-goal-braking-weight",
        type=float,
        default=0.0,
        help=(
            "execution-ranking terminal penalty on positive axial velocity, "
            "smoothly gated on near the goal"
        ),
    )
    parser.add_argument(
        "--execution-goal-braking-distance-m",
        type=float,
        default=0.60,
        help="remaining axial distance where the braking gate is half active",
    )
    parser.add_argument(
        "--execution-goal-braking-temperature-m",
        type=float,
        default=0.15,
        help="spatial temperature of the smooth goal-braking gate",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--verifier-full-h-taskspace",
        action="store_true",
        help=(
            "require every governed dense substep in the full H-step plan to "
            "remain inside the configured task space; omitted preserves the "
            "legacy first-executed-step task-space gate"
        ),
    )
    parser.add_argument(
        "--verifier-taskspace-stopping-margin-m",
        dest="verifier_stopping_margin_m",
        type=float,
        default=None,
        help=(
            "optional nonnegative inward face margin for a deterministic "
            "governor-faithful terminal stopping backup; omitted disables it"
        ),
    )
    parser.add_argument(
        "--event-log",
    ...
    parser.add_argument(
        "--future-sampling-seed", type=int, default=None,
        help=(
            "optional future-only seed for counter-based scene reset, "
            "candidate gathering, and random-success selection; exact resume "
            "keeps model, Adam, GP, replay, calibration, and stored RNG state"
        ),
    )
    args = parser.parse_args()
    if args.no_optimizer_steps_total:
        if args.optimizer_steps_per_round is None:
            parser.error(
                "--no-optimizer-steps-total requires "
    ...
    if args.helios:
        status = run_expansion_on_helios(
            ...
            script_name="research_ball_expansion_optimization.py",
        )
        _write_reproduction_command(args.output)
        return status
    if args.resume_from is not None:
        source_manifest_path = args.resume_from / "manifest.json"
        if source_manifest_path.is_file():
            source_manifest = json.loads(source_manifest_path.read_text())
            source_verifier = source_manifest.get("lab_verifier", {})
            source_full_h = bool(
                source_verifier.get("unexecuted_tail_taskspace_gate", False)
            )
            source_stopping = source_verifier.get(
                "taskspace_stopping_backup",
                source_verifier.get("stopping_backup", {}),
            ).get("face_margin_m")
            source_goal_box = source_manifest.get(
                "lab_execution_cost", {}
            ).get("execution_goal_box_exponential", {})
            source_goal_weight = float(source_goal_box.get("weight", 0.0))
            source_goal_extent = float(source_goal_box.get(
                "half_extent_m", args.execution_goal_box_half_extent_m,
            ))
            source_goal_temperature = float(source_goal_box.get(
                "temperature_m", args.execution_goal_box_exp_temperature_m,
            ))
            source_goal_side_wall = source_manifest.get(
                "lab_execution_cost", {}
            ).get("execution_goal_side_wall_quadratic")
            current_contract = (
                bool(args.verifier_full_h_taskspace),
                args.verifier_stopping_margin_m,
                float(args.execution_goal_box_exp_weight),
                float(args.execution_goal_box_half_extent_m),
                float(args.execution_goal_box_exp_temperature_m),
            )
            source_contract = (
                source_full_h,
                source_stopping,
                source_goal_weight,
                source_goal_extent,
                source_goal_temperature,
            )
            if current_contract != source_contract:
                parser.error(
                    "resume cannot change full-H task-space, task-space "
                    "stopping, or goal-box execution-cost semantics; branch "
                    "to a new output instead"
                )
            if source_goal_side_wall is not None:
                current_goal_side_wall = (
                    float(args.execution_goal_side_wall_quadratic_weight),
                    float(args.execution_goal_side_wall_target_m),
                )
                source_goal_side_wall_contract = (
                    float(source_goal_side_wall.get("weight", 0.0)),
                    float(source_goal_side_wall.get(
                        "target_m",
                        args.execution_goal_side_wall_target_m,
                    )),
                )
                if current_goal_side_wall != source_goal_side_wall_contract:
                    parser.error(
                        "resume cannot change goal-side wall execution-cost "
                        "semantics; branch to a new output instead"
                    )
        elif (
            args.verifier_full_h_taskspace
            or args.verifier_stopping_margin_m is not None
            or args.execution_goal_box_exp_weight != 0.0
        ):
            parser.error(
                "resume with new verifier/goal-box semantics requires a "
    ...
    if (
        ...
        and args.execution_rule not in {
            "min_cost", "exponential_cost", "quadratic_cost",
        }
    ):
        parser.error(
            "--execution-obstacle-speed-weight requires a cost-ranked "
            "--execution-rule"
        )
    for option, value in (
        ("--execution-soft-clearance-weight",
        ...
        ("--execution-terminal-goal-weight",
         args.execution_terminal_goal_weight),
        ("--execution-taskspace-weight",
        ...
        ("--execution-goal-side-wall-quadratic-weight",
         args.execution_goal_side_wall_quadratic_weight),
        ("--execution-goal-side-wall-target-m",
         args.execution_goal_side_wall_target_m),
        ("--execution-goal-box-exp-weight",
         args.execution_goal_box_exp_weight),
        ("--execution-goal-box-half-extent-m",
         args.execution_goal_box_half_extent_m),
        ("--execution-axis-cylinder-quadratic-weight",
         args.execution_axis_cylinder_quadratic_weight),
        ("--execution-goal-braking-weight",
         args.execution_goal_braking_weight),
        ("--execution-goal-braking-distance-m",
         args.execution_goal_braking_distance_m),
    ):
    ...
    if (
        not np.isfinite(args.execution_goal_box_exp_temperature_m)
        or args.execution_goal_box_exp_temperature_m <= 0.0
    ):
        parser.error(
            "--execution-goal-box-exp-temperature-m must be finite and positive"
        )
    if args.verifier_stopping_margin_m is not None and (
        not np.isfinite(args.verifier_stopping_margin_m)
        or args.verifier_stopping_margin_m < 0.0
    ):
        parser.error(
            "--verifier-taskspace-stopping-margin-m must be finite and "
            "nonnegative"
        )
    if (
        args.verifier_stopping_margin_m is not None
        and not args.verifier_full_h_taskspace
    ):
        parser.error(
            "--verifier-taskspace-stopping-margin-m requires "
            "--verifier-full-h-taskspace"
        )
    if (
    ...
    if (
        not np.isfinite(args.execution_goal_braking_temperature_m)
        or args.execution_goal_braking_temperature_m <= 0.0
    ):
        parser.error(
            "--execution-goal-braking-temperature-m must be finite and positive"
        )
    if (
    ...
    if (
        ...
        parser.error(
            "--execution-step-margin-weight requires a cost-ranked "
            "--execution-rule"
        )
    if (
    ...
    if (
        ...
        parser.error(
            "--execution-cost-band-fraction requires a cost-ranked "
            "--execution-rule"
        )
    if (
        args.execution_cost_band_fraction > 0.0
    ...
    if lab_profile:
        try:
            ...
            if (
                clutter_profile
                and args.execution_clearance_exp_aggregation != "mean"
            ):
                parser.error(
                    "--execution-clearance-exp-aggregation other than mean "
                    "is supported only by the single-sphere lab task"
                )
            if clutter_profile and args.execution_obstacle_speed_weight > 0.0:
                parser.error(
                    "--execution-obstacle-speed-weight is supported only by "
                    "the single-sphere lab task"
                )
            if clutter_profile:
                ...
                task = LabFlowExpansionTask(
                    ...
                    execution_clearance_exp_aggregation=(
                        args.execution_clearance_exp_aggregation
                    ),
                    execution_clearance_target_m=(
                        args.execution_clearance_target_m
                    ),
                    ...
                    execution_terminal_goal_weight=(
                        args.execution_terminal_goal_weight
                    ),
                    execution_taskspace_weight=(
                        args.execution_taskspace_weight
                    ...
                    execution_goal_box_exp_temperature_m=(
                        args.execution_goal_box_exp_temperature_m
                    ),
                    ...
                    execution_axis_cylinder_finite_segment=(
                        args.execution_axis_cylinder_finite_segment
                    ),
                    ...
                    execution_goal_braking_temperature_m=(
                        args.execution_goal_braking_temperature_m
                    ),
    ...
    if args.sample_update_submodes == "angular8":
        if args.successful_trajectories_per_gamma != 8:
            parser.error(
                "--sample-update-submodes angular8 requires "
                "--successful-trajectories-per-gamma 8"
            )
        sample_update_mode = tuple(range(8))
    elif args.sample_update_mode is not None:
        try:
    ...
    config = ExpansionConfig(
        ...
        execution_step_margin_mode=args.execution_step_margin_mode,
        execution_cost_band_fraction=args.execution_cost_band_fraction,
        execution_cost_band_margin=args.execution_cost_band_margin,
        ...
        sample_update_cohorts=args.sample_update_cohorts,
    ...
    fa_stats = {
        ...
        "sample_update_retry_guidance": (
            (
                ...
                if args.sample_update_cohorts == "paired"
    ...
    manifest["ball_verifier_bounds"] = {
        ...
        "contract": (
            "every governed dense substep in the full H-step plan must remain "
            "inside taskspace"
            if getattr(task, "verifier_full_h_taskspace", False)
            else "only the dense first executed segment must remain inside "
            "taskspace; the unexecuted H-1 tail is not taskspace-gated"
        ),
    }
    ...
    if lab_profile:
        ...
        manifest["lab_execution_cost"] = {
            ...
            "step_margin_blend": {
                ...
                "mode": args.execution_step_margin_mode,
                "formula": (
                    "J_execution - weight * first_step_nominal_H_P_margin"
                    if args.execution_step_margin_mode == "linear"
                    else "normalized(J_execution) - weight * "
                    "normalized(first_step_nominal_H_P_margin)"
                ),
                "scope": (
                    "ranking among the same verifier-eligible B candidates; "
                    "may be combined with the declared obstacle cost"
                ),
            },
            "cost_band_margin_hybrid": {
                "fraction": float(args.execution_cost_band_fraction),
                "margin": args.execution_cost_band_margin,
                "formula": (
                    "shortlist J <= J_min + fraction*(J_max-J_min), then "
                    "maximize the declared margin with minimum-cost tie break"
                ),
            },
            ...
            "execution_goal_side_wall_quadratic": {
                ...
                "target_m": float(args.execution_goal_side_wall_target_m),
                "faces": ["x_max", "y_min"],
                "formula": (
                    "weight * sum_faces max(target_m-"
                    "terminal_signed_face_clearance,0)^2"
                ),
                "predicted_states": "terminal H post-action plan knot only",
                "scope": (
                    "execution ranking only; physical task-space, full-H "
                    "verifier, rollout OOB, and goal radius unchanged"
                ),
            },
            "execution_goal_box_exponential": {
                "weight": float(args.execution_goal_box_exp_weight),
                "half_extent_m": float(
                    args.execution_goal_box_half_extent_m
                ),
                "temperature_m": float(
                ...
                "effective_bounds": {
                    "lower": (
                        task.env.goal - args.execution_goal_box_half_extent_m
                    ).tolist(),
                    "upper": (
                        task.env.goal + args.execution_goal_box_half_extent_m
                    ).tolist(),
                ...
                "formula": (
                    "weight * mean_h sum_xyz expm1(clamp(" 
                    "max(abs(p_h-goal)-half_extent_m,0)/temperature_m,0,20))"
                ),
                "predicted_states": "H post-action plan knots",
                "scope": (
                    "execution ranking only; physical task-space, verifier "
                    "bounds, OOB termination, and goal radius unchanged"
                ),
            },
            "execution_axis_cylinder_quadratic": {
                ...
                "formula": (
                    "weight * ("
                    "terminal_distance_to_start_goal_"
                    "segment_or_axis/radius_m)^2"
                ),
                "predicted_states": "H post-action plan knots",
                "scope": (
                    "execution ranking only; finite_segment optionally adds "
                    "start/goal end caps; verifier and rollout unchanged"
                ),
            },
            "execution_control": {
                "configured_weight": float(
                ...
                "formula": "weight * sum_h ||raw_action_h||^2",
                "scope": "execution ranking only",
            },
            "execution_terminal_goal": {
                "configured_weight": float(
                    task_config.safemppi.terminal_goal_weight
                ),
                "effective_weight": float(
                    getattr(
                        task,
                ...
                "formula": "weight * ||terminal_position-goal||^2",
                "scope": "execution ranking only",
            },
            "execution_goal_braking": {
                "weight": float(args.execution_goal_braking_weight),
                "distance_m": float(args.execution_goal_braking_distance_m),
                "temperature_m": float(
                    args.execution_goal_braking_temperature_m
                ),
                "formula": (
                    "weight * sigmoid((distance_m-remaining_axial)/"
                    "temperature_m) * max(terminal_axial_velocity,0)^2"
                ),
                "scope": "execution ranking only; verifier unchanged",
            },
            "execution_clearance_exponential": {
                ...
                "aggregation": args.execution_clearance_exp_aggregation,
                "formula": {
                    "mean": (
                        "weight * mean_h "
                        "exp((target_m-clearance_h)/temperature)"
                    ),
                    "max": (
                        "weight * max_h "
                        "exp((target_m-clearance_h)/temperature)"
                    ...
                    "top3_mean": (
                        "weight * mean_largest_min(3,H)_h "
                        "exp((target_m-clearance_h)/temperature)"
                    ),
                }[args.execution_clearance_exp_aggregation],
                "predicted_states": "H post-action plan knots",
            ...
            "execution_obstacle_conditioned_speed": {
                ...
                "predicted_states": "H post-action plan knots",
                "scope": (
                    "expansion execution ranking only; raw deployment "
                    "evaluation, verifier, and rollout dynamics unchanged"
                ),
            },
        ...
        manifest["lab_verifier"] = {
            ...
            "unexecuted_tail_taskspace_gate": bool(
                args.verifier_full_h_taskspace
            ),
            "taskspace_gate_scope": (
                "full_h_dense"
                if args.verifier_full_h_taskspace
                else "executed_first_step_dense"
            ),
            "taskspace_stopping_backup": {
                "enabled": args.verifier_stopping_margin_m is not None,
                "face_margin_m": args.verifier_stopping_margin_m,
                "contract": (
                    "deterministic feasible governor backup with exact "
                    "deployment smoothing; sufficient task-space recursive-"
                    "feasibility check, not a collision or optimal viability "
                    "certificate"
                ),
            },
        ...
        manifest["lab_coverage_objective"] = {
            ...
            "sample_update_retry_guidance": (
                (
                    ...
                    "fa-alloc differs; both cohorts share one route quota and "
                    "stop together"
                    if args.sample_update_cohorts == "paired"
