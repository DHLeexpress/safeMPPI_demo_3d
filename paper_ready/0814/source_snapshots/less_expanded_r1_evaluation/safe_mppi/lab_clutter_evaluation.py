...
from .lab_clutter_pre2_expansion import bowling_123_spheres
...
PERPENDICULAR_TRIANGLE_TASK_PROFILE = (
    "minhyuk_lab_perpendicular_triangle_sphere_visual_expansion"
)
...
PATH_FOCUSED_DOUBLE_HOURGLASS_SPHERE_SCENE_SCHEMA = (
    "lab_path_focused_double_hourglass_variable_spheres_v1"
)
PATH_FOCUSED_CENTER_ANCHORED_SPHERE_SCENE_SCHEMA = (
    "lab_path_focused_center_anchored_truncnorm_variable_spheres_v1"
)
...
def is_lab_clutter_evaluation_manifest(manifest: dict) -> bool:
    ...
    expected_schemas = {
        LAB_CLUTTER_TASK_PROFILE: frozenset({LAB_CLUTTER_SCENE_SCHEMA}),
        PERPENDICULAR_TRIANGLE_TASK_PROFILE: frozenset({
            PERPENDICULAR_TRIANGLE_SCENE_SCHEMA,
        }),
        PATH_FOCUSED_SPHERE_TASK_PROFILE: PATH_FOCUSED_SPHERE_SCENE_SCHEMAS,
    }[profile]
    if (
        len(schemas) != 1
...
def _scene_record_from_spheres(
    scene_spec,
    env: TaskEnvironment,
    spheres: np.ndarray,
    *,
    ...
    """Serialize explicitly constructed geometry under the sampler contract."""
    validated = scene_spec.validate(env, spheres)
    scene_hash = scene_sha256(env, validated)
    scene = ClutterScene(
        index=int(scene_seed),
        seed=int(scene_seed),
        spheres=tuple(tuple(map(float, row)) for row in validated),
        cylinders=(),
        scene_hash=scene_hash,
    )
    row = {
        "scene_seed": int(scene_seed),
        "scene_hash": scene_hash,
        "obstacle_count": int(len(validated)),
        "spheres": validated.tolist(),
        "start_goal_path_diagnostics": start_goal_path_diagnostics(
            scene,
            start=env.start,
            goal=env.goal,
...
def _fixed_evaluation_scene_bank(
    ...
    sampler_contract = (
        scene_spec.sampling_contract()
        if hasattr(scene_spec, "sampling_contract") else None
    )
    return {
        ...
        "sampler": {
            ...
            "distribution_contract": sampler_contract,
        },
...
def _fixed_scene_provenance(
    ...
    if mode == "sampled":
        ...
        construction = "independent sample from configured scene law"
    elif mode == "triangle_base":
        if not isinstance(scene_spec, PerpendicularTriangleSphereScene):
            raise ValueError(
                "triangle_base fixed scene requires the perpendicular-triangle "
                "scene distribution"
            )
        scene = _scene_record_from_spheres(
            scene_spec,
            env,
            scene_spec.base_scene(env),
            scene_seed=int(scene_seed),
        )
        construction = (
            "unperturbed equilateral triangle in the plane perpendicular "
            "to the start-goal axis"
        )
    elif mode == "bowling_123":
        scene = _scene_record_from_spheres(
            scene_spec,
            env,
            bowling_123_spheres(
        ...
        construction = (
            "fixed six-sphere 1-2-3 bowling layout at progress "
            "0.25/0.45/0.65 with 0.345 m lane spacing"
        )
    else:
        raise ValueError(f"unknown fixed scene mode: {mode}")
    expansion_hashes = _expansion_scene_hashes(manifest)
    randomized_hashes = {
...
def _optional_start_probe(
    ...
    *,
    ...
    """Skip the auxiliary probe explicitly without affecting raw rollouts."""
    if int(samples) < 0:
        raise ValueError("--probe-samples must be nonnegative")
    if int(samples) == 0:
        return []
    device_kwargs = (
        {} if torch.device(device).type == "cpu" else {"device": device}
    )
    return _start_probe(
        policy,
        config,
...
def _triangle_plane_crossing(
    states: np.ndarray,
    center: np.ndarray,
    frame: np.ndarray,
) -> np.ndarray:
    """Return a path's interpolated crossing of the triangle's normal plane."""
    local = (np.asarray(states, float)[:, :3] - center[None]) @ frame
    longitudinal = local[:, 0]
    exact = np.flatnonzero(np.isclose(longitudinal, 0.0, atol=1.0e-12))
    if len(exact):
        return local[int(exact[0]), 1:3]
    changes = np.flatnonzero(longitudinal[:-1] * longitudinal[1:] < 0.0)
    if len(changes):
        index = int(changes[0])
        fraction = float(
            -longitudinal[index]
            / (longitudinal[index + 1] - longitudinal[index])
        )
        return (
            local[index, 1:3]
            + fraction * (local[index + 1, 1:3] - local[index, 1:3])
        )
    return local[int(np.argmin(np.abs(longitudinal))), 1:3]


def _plot_fixed_scene_crossing_coverage(
    per_round_rows,
    summaries,
    rounds,
    gammas,
    ...
    """Show center/around-route support where paths meet the triangle plane."""
    spheres = np.asarray(scene["spheres"], float)
    center = spheres[:, :3].mean(axis=0)
    env = TaskEnvironment(config)
    frame = start_goal_frame(env).astype(float)
    local_centers = (spheres[:, :3] - center[None]) @ frame
    physical_radius = float(
        config.raw["scene_randomization"]["physical_radius_m"]
    )
    modeled_radius = float(spheres[0, 3])
    fig, axes = plt.subplots(
        len(rounds), len(gammas),
        figsize=(4.0 * len(gammas), 3.7 * len(rounds)),
        squeeze=False,
        sharex=True,
        sharey=True,
    )
    ...
    for row_index, round_i in enumerate(rounds):
        for column_index, gamma in enumerate(gammas):
            ...
            for sphere_center in local_centers:
                for radius, color, style, alpha in (
                    (modeled_radius, "#8e969e", "--", 0.65),
                    (physical_radius, "#5c6266", "-", 0.90),
                ):
                    axis.plot(
                        sphere_center[1] + radius * np.cos(theta),
                        sphere_center[2] + radius * np.sin(theta),
                        color=color,
                        linestyle=style,
                        linewidth=1.0,
                        alpha=alpha,
            ...
            for record in records:
                crossing = _triangle_plane_crossing(
                    record["states"], center, frame,
                )
                success = record["status"] == "SUCCESS"
                axis.scatter(
                    crossing[0], crossing[1],
                    marker=("o" if success else "x"),
                    s=(24 if success else 30),
                    color=("#1076a8" if success else "#c8321b"),
                    alpha=(0.72 if success else 0.65),
                    linewidths=1.0,
                )
            axis.scatter(0.0, 0.0, marker="+", s=55, color="black")
            cell = summaries[str(int(round_i))]["per_gamma"][f"{gamma:g}"]
            spread = cell["successful_path_spread_m"]
            axis.set_title(
                rf"$\gamma={gamma:g}$ | SR={cell['SR']:.2f} | "
                + (rf"spread={spread:.3f} m" if spread is not None else "spread=n/a"),
                fontsize=10,
            )
            axis.set_aspect("equal", adjustable="box")
            axis.grid(alpha=0.20)
            if row_index == len(rounds) - 1:
                axis.set_xlabel(r"lateral $e_1$ [m]")
            if column_index == 0:
                axis.set_ylabel(rf"round {round_i}\nvertical $e_2$ [m]")
    fig.suptitle(
        "Raw-policy crossings of the fixed tight-triangle plane\n"
        "circles: physical (solid) / robot-inflated (dashed); "
...
def evaluate_lab_clutter_expansion(
    ...
    if fixed_scene_rollouts < 1:
        raise ValueError("--fixed-scene-rollouts must be positive")
    requested_fixed_mode = str(
        getattr(args, "fixed_scene_mode", "auto")
    )
    if requested_fixed_mode == "auto":
        fixed_scene_mode = (
            "triangle_base"
            if isinstance(
                sphere_scene_spec_from_config(config),
                PerpendicularTriangleSphereScene,
            )
            else "sampled"
        )
    elif requested_fixed_mode in {
        "sampled", "triangle_base", "bowling_123",
    }:
        fixed_scene_mode = requested_fixed_mode
    else:
        raise ValueError(
            "--fixed-scene-mode must be auto, sampled, triangle_base, "
            "or bowling_123"
        )
    ...
    for progress_index, round_i in enumerate(rounds, start=1):
        ...
        if fixed_scene_mode == "bowling_123":
            sphere_radius = float(
                fixed_scene["scene"]["spheres"][0][3]
            )
        ...
        probe_rows = _optional_start_probe(
            policy,
            config,
            gammas,
        ...
        summaries[str(round_i)] = {
            ...
            "start_probe_validity": (
                float(np.mean([row["valid"] for row in probe_rows]))
                if probe_rows
                else None
            ),
        }
        ...
        if fixed_scene_mode == "bowling_123":
            fixed_summaries[str(round_i)]["bowling_coverage"] = {
                "pooled": summarize_bowling_coverage(fixed_rows),
                "per_gamma": {
                    f"{gamma:g}": summarize_bowling_coverage([
                        row for row in fixed_rows
                        if row["gamma"] == gamma
                    ])
                    for gamma in gammas
    ...
    (output / "fixed_scene_raw_eval.json").write_text(json.dumps({
        ...
        "bowling_coverage_contract": (
            {
                "route_codes": list(BOWLING_ROUTE_CODES),
                "classifier": (
                    "SUCCESS-only first-forward crossings at progress "
                    "0.25/0.45/0.65; sequential projected L/R with "
                    "0.02 m stability margin"
                ),
                "coverage_denominator": len(BOWLING_ROUTE_CODES),
                "full_3d_homotopy_claimed": False,
            }
            if fixed_scene_mode == "bowling_123" else None
        ),
    ...
    # The canonical raw curves answer the fixed-challenge question: every
    # checkpoint is evaluated on the same unjittered nominal triangle.  Keep
    # randomized-domain curves as a separately named generalization audit.
    _plot_curves(
        fixed_summaries, gammas, output / "raw_curves.png",
    )
    _plot_curves(
        summaries, gammas, output / "randomized_raw_curves.png",
    )
    _plot_count_curves(
    ...
    if fixed_scene["mode"] == "triangle_base":
        _plot_fixed_scene_crossing_coverage(
            fixed_per_round_rows,
            fixed_summaries,
            display_rounds,
            gammas,
            config,
            fixed_scene["scene"],
            output / "fixed_scene_crossing_coverage.png",
        )
