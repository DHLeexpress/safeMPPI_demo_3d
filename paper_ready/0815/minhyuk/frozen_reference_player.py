#!/usr/bin/env python3
"""Validated 100 Hz playback core for one frozen 0815 reference.
...
def load_reference(
    ...
    *,
    ...
    required = {"time_s", "position_ref", "velocity_ref", "acceleration_ref"}
    ...
    if any(not np.isfinite(reference[key]).all() for key in required):
        raise ValueError("reference contains non-finite values")
    status = str(reference.get("status", np.asarray("UNKNOWN")).item())
    hardware_eligible = bool(
        reference.get("hardware_eligible", np.asarray(False)).item()
    )
    if not allow_simulation_only and (
        status != "SUCCESS" or not hardware_eligible
    ):
        raise ValueError(
            f"reference is simulation-only: status={status}, "
            f"hardware_eligible={hardware_eligible}"
        )
...
def stream_reference(
    ...
    *,
    ...
    if str(reference.get("status", np.asarray("UNKNOWN")).item()) != "SUCCESS":
        raise ValueError("refusing to stream a non-success reference")
    if not bool(reference.get("hardware_eligible", np.asarray(False)).item()):
        raise ValueError("refusing to stream a simulation-only reference")
    start = now()
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="validate a DO_NOT_FLY reference without enabling streaming",
    )
    args = parser.parse_args()
    reference = load_reference(
        args.reference,
        args.expected_sha256,
        allow_simulation_only=args.allow_simulation_only,
    )
    report = {
